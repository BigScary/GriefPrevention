...
class PlayerEventHandler implements Listener {
	...
	private boolean handlePlayerChat(Player player, String message,
		...
		if (this.howToClaimPattern.matcher(message).matches()) {
			...
			final PlayerData pdata = GriefPrevention.instance.dataStore
		...
		if (message.length() > 4
				&& this.stringsAreSimilar(message.toUpperCase(), message)) {
		...
		long millisecondsSinceLastMessage = (new Date()).getTime()
				- playerData.lastMessageTimestamp.getTime();
		...
		if (!muted) {
			Pattern ipAddressPattern = Pattern
					.compile("\\d{1,4}\\D{1,3}\\d{1,4}\\D{1,3}\\d{1,4}\\D{1,3}\\d{1,4}");
			...
			if (matcher.find()) {
				...
				if (!wc.getSpamAllowedIpAddresses().contains(matcher.group())) {
					// log entry
					GriefPrevention.AddLogEntry("Muted IP address from "
		...
		if (!muted && message.length() > wc.getSpamCapsMinLength()) {
			...
			try {
				for (int i = 0; i < message.length()
						- wc.getSpamCapsMinLength(); i++) {
					String teststr = "";
					try {
						teststr = message.substring(i,
								i + wc.getSpamCapsMinLength());
					} catch (StringIndexOutOfBoundsException exx) {
						// testing code for ticket 212
						// print out diagnostic info.
						exx.printStackTrace();
						System.out
								.println("Unexpected Error Processing Chat Command");
						System.out.println("Current Message:\"" + message
								+ "\"");
						System.out.println("SpamCapsMinLength:"
								+ wc.getSpamCapsMinLength());
		...
		if (!muted && message.length() > wc.getSpamNonAlphaNumMinLength()) {
			...
			if (symbolsCount > message.length() / 2
					|| (message.length() > wc.getSpamASCIIArtMinLength() && whitespaceCount < message
							.length() / 10)) {
				spam = true;
		...
		if (!muted
				&& message.length() < wc.getSpamShortMessageMaxLength()
				&& millisecondsSinceLastMessage < wc
						.getSpamShortMessageTimeout()) {
			spam = true;
			playerData.spamCount++;
		}
		...
		if (spam) {
			if (playerData.spamCount > wc.getSpamKickThreshold()
					&& playerData.spamWarned) {
				// log entry
			...
			if (playerData.spamCount > wc.getSpamBanThreshold()
					&& wc.getSpamBanThreshold() > 0 && playerData.spamWarned) {
				// log entry
				GriefPrevention.AddLogEntry("Acting on " + player.getName()
						+ " for spam.");

				// kick and ban
				PlayerKickBanTask task = new PlayerKickBanTask(player,
						wc.getSpamBanMessage());
				GriefPrevention.instance
						.getServer()
						.getScheduler()
						.scheduleSyncDelayedTask(GriefPrevention.instance,
			...
			if (playerData.spamCount >= wc.getSpamMuteThreshold()) {
				...
				if (!playerData.spamWarned) {
					GriefPrevention.sendMessage(player, TextMode.Warn,
							wc.getSpamWarningMessage(), 10L);
					GriefPrevention.AddLogEntry("Warned " + player.getName()
			...
			if (muted) {
				// make a log entry
				GriefPrevention.AddLogEntry("Muted spam (spamcount:"
						+ playerData.spamCount + ") from " + player.getName()
						+ ": " + message);
	...
	private boolean stringsAreSimilar(String message, String lastMessage) {
		...
		int maxIdenticalCharacters = longerString.length()
				- longerString.length() / 4;
		...
		// compare backward
		for (int i = 0; i < shorterString.length(); i++) {
			if (shorterString.charAt(shorterString.length() - i - 1) == longerString
					.charAt(longerString.length() - i - 1))
				identicalCount++;
			if (identicalCount > maxIdenticalCharacters)
				return true;
		}
	...
	synchronized void onPlayerCommandPreprocess(
		...
		if (event.getPlayer() == null)
			return;
		WorldConfig wc = GriefPrevention.instance.getWorldCfg(event.getPlayer()
				.getWorld());
		if (wc == null)
			return;
		if (!wc.getClaimsEnabled())
		...
		if (wc.getEavesDrop() && WhisperCommands.contains(command)
				&& args.length > 1) {
			StringBuilder logMessageBuilder = new StringBuilder();
			logMessageBuilder.append("[[").append(event.getPlayer().getName())
					.append("]] ");
		...
		if ((playerData.inPvpCombat() || playerData.siegeData != null)
				&& wc.getPvPBlockedCommands().contains(command)) {
			event.setCancelled(true);
	...
	void onPlayerLogin(PlayerLoginEvent event) {
		...
		if (wc.getSpamProtectionEnabled()) {
			...
			if (wc.getSpamLoginCooldownSeconds() > 0
				...
				PlayerData playerData = this.dataStore.getPlayerData(player
						.getName());
				long millisecondsSinceLastLogin = (new Date()).getTime()
						- playerData.lastLogin.getTime();
				long secondsSinceLastLogin = millisecondsSinceLastLogin / 1000;
				long cooldownRemaining = wc.getSpamLoginCooldownSeconds()
						- secondsSinceLastLogin;

				// if cooldown remaining and player doesn't have permission to
				// spam
				if (cooldownRemaining > 0
						&& !player.hasPermission("griefprevention.loginspam")) {
					// DAS BOOT!
					event.setResult(Result.KICK_OTHER);
					String cooldown = GriefPrevention.instance.dataStore
							.getMessage(Messages.LoginSpamWaitSeconds,
									Long.toString(cooldownRemaining));
				...
				// if we find a match
					...
					// account, too
						...
						for (int k = 0; k < players.length; k++) {
							if (players[k].isOp()) {
								GriefPrevention.sendMessage(players[k],
										TextMode.Success,
	...
	void onPlayerDeath(PlayerDeathEvent event) {
		WorldConfig wc = GriefPrevention.instance.getWorldCfg(event.getEntity()
				.getWorld());
		if (!wc.getClaimsEnabled())
			return;
		...
		PlayerData playerData = this.dataStore.getPlayerData(event.getEntity()
				.getName());
		long now = Calendar.getInstance().getTimeInMillis();
		if (now - playerData.lastDeathTimeStamp < wc
				.getSpamDeathMessageCooldownSeconds() * 1000) {
	...
	public void onPlayerTeleport(PlayerTeleportEvent event) {
		...
		if (event.getCause() == TeleportCause.ENDER_PEARL) {
			if (wc.getEnderPearlOrigins().Allowed(event.getFrom(), player)
					.Denied()) {
			...
			else if (wc.getEnderPearlTargets().Allowed(event.getTo(), player)
					.Denied()) {
				player.getInventory().addItem(
						new ItemStack(Material.ENDER_PEARL, 1));
				event.setCancelled(true);
	...
	public void onPlayerShearEntity(PlayerShearEntityEvent event) {
		...
		if (wc.getShearingRules().Allowed(entity.getLocation(), player)
				.Denied()) {
			event.setCancelled(true);
		}
		...
		try {
			...
			if (entity instanceof Creature) {
				...
				if (handItem.getType() == Material.LEASH) {
					...
					if (wc.getLeadUsageRules()
							.Allowed(entity.getLocation(), player).Denied()) {
						event.setCancelled(true);
						return;
					}
			...
			if (isHorse(entity)) {
				...
				if (h.isTamed() && handItem.getType() == Material.GOLDEN_APPLE) {
					...
					if (wc.getBreedingRules().Allowed(h.getLocation(), player)
							.Denied()) {
						event.setCancelled(true);
						return;
					}

				} else if (handItem.getType() == Material.WHEAT
						|| handItem.getType() == Material.HAY_BLOCK
						|| handItem.getType() == Material.APPLE
						|| handItem.getType() == Material.GOLDEN_APPLE) {
					// apply feeding rules.
					if (wc.getFeedingRules().Allowed(h.getLocation(), player)
							.Denied()) {
						event.setCancelled(true);
						return;
					}
					...
					if (h.isTamed()) {
						...
						if (player.isSneaking()) {
							if (wc.getEquineInventoryRules()
									.Allowed(h.getLocation(), player).Denied())
								event.setCancelled(true);
							return;
			...
			if (entity instanceof Pig || entity instanceof Sheep
					|| entity instanceof Cow || entity instanceof MushroomCow
					|| entity instanceof Chicken) {
				// is the player holding the breeding item for this entity?
				List<Material> breeder = new ArrayList<Material>();
				for (Material iterate : GriefPrevention.instance
						.getEntityBreedingItems(entity)) {
					breeder.add(iterate);
				}
				if (breeder.contains(handItem.getType())) {

					// can we breed?
					if (wc.getBreedingRules()
							.Allowed(entity.getLocation(), player).Denied()) {
						event.setCancelled(true);
				...
				if (tamed.isTamed()) {
					...
					if (cr.getMaxHealth() == cr.getHealth()) {
						...
						if (breeder.contains(handItem)) {
							...
							if (wc.getBreedingRules()
									.Allowed(entity.getLocation(), player)
									.Denied()) {
								event.setCancelled(true);
								return;
							}
			...
			if (entity instanceof Villager
					&& wc.getVillagerTrades()
							.Allowed(entity.getLocation(), player).Denied()) {

				event.setCancelled(true);
				return;
			}
			if ((entity instanceof Sheep && event.getPlayer().getItemInHand()
					.getType() == Material.INK_SACK)) {
				// apply dyeing rules.
				if (wc.getSheepDyeingRules()
						.Allowed(entity.getLocation(), event.getPlayer())
						.Denied()) {
					event.setCancelled(true);
					return;
				}
			...
			if ((entity instanceof StorageMinecart
					|| entity instanceof PoweredMinecart || entity instanceof HopperMinecart)) {
				if (playerData.siegeData != null) {
					GriefPrevention.sendMessage(player, TextMode.Err,
			...
			if (wc.getContainersRules()
					.Allowed(entity.getLocation(), player, false).Denied()
				...
				if (claim != null) {
					...
					if (entity instanceof StorageMinecart
						...
						String noContainersReason = claim
								.allowContainers(player);
			...
			Debugger.Write(
					"PlayerInteractEntity: Cancelled:" + event.isCancelled(),
					DebugLevel.Verbose);
		}
	}
			...
			if (player.hasPermission("GriefPrevention.Administration")) {
				GriefPrevention
						.sendMessage(
								player,
								TextMode.Info,
								"GriefPrevention Admin tool selected. Left-Click to add to container list. Shift Left-click to add to only the current world.");
			}
	...
	public void onPlayerBedEnter(PlayerBedEnterEvent bedEvent) {
		...
		if (!wc.getClaimsEnabled())
			return;
		ClaimAllowanceConstants resultdata = wc.getBeds().Allowed(
				block.getLocation(), bedEvent.getPlayer(), false);
		...
		// if the bed is in a claim
			...
			Claim grabclaim = GriefPrevention.instance.dataStore.getClaimAt(
					block.getLocation(), true);
			bedEvent.setCancelled(true);
			GriefPrevention.sendMessage(player, TextMode.Err,
					Messages.NoBedPermission, grabclaim.getOwnerName());
	...
	public void onPlayerBucketEmpty(PlayerBucketEmptyEvent bucketEvent) {
		...
		if (bucketEvent.getBucket() == Material.LAVA_BUCKET) {
			switch (wc.getLavaBucketEmptyBehaviour().Allowed(
			...
			switch (wc.getWaterBucketEmptyBehaviour().Allowed(
					block.getLocation(), player, false)) {
			case Allow_Forced:
				return; // force allow.
			case Deny_Forced:
				bucketEvent.setCancelled(true);
		...
		if (bucketEvent.getBucket() == Material.LAVA_BUCKET) {
			if (wc.getLavaBucketEmptyBehaviour()
					.Allowed(block.getLocation(), player).Denied()) {
				// GriefPrevention.sendMessage(player, TextMode.Err,
			...
			if (wc.getWaterBucketEmptyBehaviour()
					.Allowed(block.getLocation(), player).Denied()) {
				// GriefPrevention.sendMessage(player, TextMode.Err,
	...
	public void onPlayerBucketFill(PlayerBucketFillEvent bucketEvent) {
		try {
			...
			WorldConfig wc = GriefPrevention.instance.getWorldCfg(block
					.getWorld());
			if (!wc.getClaimsEnabled())
				return;
			// make sure the player is allowed to build at the location
			...
			if (bucketEvent.isCancelled()) {
				...
				Bukkit.getScheduler().scheduleSyncDelayedTask(
						GriefPrevention.instance, new Runnable() {

							public void run() {

								cloned.getPlayer().sendBlockChange(
										cloned.getBlockClicked().getLocation(),
										cloned.getBlockClicked().getType(),
										cloned.getBlockClicked().getData());
							}
						}, 0);
			}
		}
	...
	private void getTransparentMaterials() {
		if (transparentMaterials == null) {
			...
			transparentMaterials
					.add(Byte.valueOf((byte) Material.SNOW.getId()));
			transparentMaterials.add(Byte.valueOf((byte) Material.LONG_GRASS
					.getId()));
			transparentMaterials.add(Byte.valueOf((byte) Material.WOOD_BUTTON
					.getId()));
			transparentMaterials.add(Byte.valueOf((byte) Material.STONE_BUTTON
					.getId()));
			transparentMaterials.add(Byte.valueOf((byte) Material.STONE_PLATE
					.getId()));
			transparentMaterials.add(Byte.valueOf((byte) Material.WOOD_PLATE
					.getId()));
			transparentMaterials.add(Byte.valueOf((byte) Material.IRON_PLATE
					.getId()));
			transparentMaterials.add(Byte.valueOf((byte) Material.GOLD_PLATE
					.getId()));
			transparentMaterials
					.add(Byte.valueOf((byte) Material.LEVER.getId()));
		}
	}
	...
	void onPlayerInteract(PlayerInteractEvent event) {
		...
		Player player = event.getPlayer();
		String ItemName = event.getItem() == null ? "null" : "ID:"
				+ String.valueOf(event.getItem().getTypeId());
		...
		if (wc.getItemRules() != null) {
			...
			try {
				clickedBlock = player
						.getTargetBlock(transparentMaterials, 2500);
			} catch (Exception exx) {
				clickedBlock = null;
			}
		...
		if (GriefPrevention.instance.DebuggingLevel == DebugLevel.Verbose) {
			...
			if (clickedBlock != null) {
				...
				if (clickedBlock.getState() != null)
					sb.append(" "
							+ clickedBlock.getState().getClass().getName());
			}
			sb.append(",");
			if (event != null) {
				if (event.getItem() != null) {
					sb.append("Item:" + event.getItem().getType().name()
							+ " ts:" + event.getItem().toString());
		...
		Material inhand = event.getItem() == null ? null : event.getItem()
				.getType();
		if (inhand != null && GPTools.contains(inhand)) {
			...
			try {
				...
				if (clickedBlock == null
					...
					getTransparentMaterials();
					clickedBlock = player.getTargetBlock(transparentMaterials,
							250);
				}
		...
		if ((clickedBlock.getType() == Material.FENCE || clickedBlock.getType() == Material.NETHER_FENCE)
			...
			if (wc.getLeadUsageRules()
					.Allowed(clickedBlock.getLocation(), player).Denied()) {
				event.setCancelled(true);
				return;
			}
		...
		if (event.getAction() == Action.LEFT_CLICK_BLOCK
			...
			if (player.hasPermission("GriefPrevention.Administration")) {
				...
				if (event.getAction() == Action.LEFT_CLICK_BLOCK) {
					if (player.isSneaking()) {
						GriefPrevention.instance.Configuration.AddContainerID(
								player, player.getWorld(), new MaterialInfo(
										event.getClickedBlock().getType()));
						event.setCancelled(true);
					} else {
						GriefPrevention.instance.Configuration.AddContainerID(
								player, null, new MaterialInfo(event
		...
		if ((event.getAction() == Action.RIGHT_CLICK_BLOCK && (clickedBlock
				.getState() instanceof InventoryHolder
				|| ContainerMaterials.contains(clickedBlock.getType()) ||

		wc.getModsContainerTrustIds().contains(
				new MaterialInfo(clickedBlock.getTypeId(), clickedBlock
						.getData(), null))))) {
			...
			Claim cc = this.dataStore.getClaimAt(clickedBlock.getLocation(),
					true);
			// if doorsOpen...
			...
			if (cc != null && cc.doorsOpen) {
				...
				if ((cc.LootedChests++) <= wc.getSeigeLootChests()
						&& wc.getSeigeLootChests() > 0) {
					// tell the player how many more chests they can loot.
					player.sendMessage(ChatColor.YELLOW + " You may loot "
							+ (wc.getSeigeLootChests() - cc.LootedChests)
							+ " more chests");
					return;
				}
			...
			if (wc.getContainersRules()
					.Allowed(clickedBlock.getLocation(), player, true).Denied()) {
		...
		if (useRule != null) {
			...
			if (useRule == wc.getWoodPressurePlates()
					|| useRule == wc.getStonePressurePlates()) {
				Date prevstepped = playerData.getLastSteppedOn(clickedBlock
						.getLocation());
				doshowmessage = prevstepped == null
						|| prevstepped.before(laststepped);
			}
			// System.out.println("doshowmessage=" + doshowmessage);
			playerData.setLastSteppedOn(clickedBlock.getLocation());

			if (useRule.Allowed(clickedBlock.getLocation(), player,
					doshowmessage).Denied()) {
				event.setCancelled(true);
				return;
			}
		...
		// can still trample.
				...
				&& clickedBlockType == Material.SOIL) {
			ClaimBehaviourData.ClaimAllowanceConstants trampleresult = wc
					.getPlayerTrampleRules().Allowed(
		...
		// apply rule for note blocks and repeaters
			...
			ClaimAllowanceConstants tweakallow = wc.getBlockTweakRules()
					.Allowed(clickedBlock.getLocation(), event.getPlayer());
		...
		// otherwise handle right click (shovel, stick, bonemeal)
			...
			if (materialInHand == Material.INK_SACK) {
				if (wc.getBonemealGrassRules()
						.Allowed(event.getClickedBlock().getLocation(),
								event.getPlayer()).Denied()) {
					event.setCancelled(true);
				}
				return;
			}
			...
			// world, apply special rules
					...
					|| materialInHand == Material.EXPLOSIVE_MINECART || materialInHand == Material.BOAT)
					&& GriefPrevention.instance.creativeRulesApply(clickedBlock
							.getLocation())) {
				// player needs build permission at this location
				String noBuildReason = GriefPrevention.instance.allowBuild(
						player, clickedBlock.getLocation());
				if (noBuildReason != null) {
					GriefPrevention.sendMessage(player, TextMode.Err,
							noBuildReason);
			...
			// if he's investigating a claim
				...
				if (player.isSneaking()
						&& player
								.hasPermission("GriefPrevention.InvestigateArea")) {
					...
					for (int X = StartX; X < EndX; X++) {
						for (int Z = StartZ; Z < EndZ; Z++) {
							...
							Claim grabclaim = dataStore.getClaimAt(
									new Location(player.getWorld(), X,
											clickedBlock.getLocation()
													.getBlockY(), Z), false);
							if (grabclaim != null
									&& !buildlist.contains(grabclaim)) {
					...
					claimsshow = new Claim[] { this.dataStore
				...
				// no claim case
				if (claimsshow == null || claimsshow.length == 0
						|| claimsshow[0] == null) {
				...
				// claim case
					...
					if (claimsshow.length == 1)
						...
						String messagedisplay = "Showing "
								+ claimsshow.length
								+ " Claims within "
								+ wc.getConfigShowSurroundingsRadius()
								+ " blocks of "
								+ GriefPrevention
										.getfriendlyLocationString(clickedBlock
												.getLocation());
						...
						String shownames = buildnames.toString().substring(0,
								buildnames.length() - 1);

						GriefPrevention.sendMessage(player, TextMode.Info,
								messagedisplay);
						GriefPrevention.sendMessage(player, TextMode.Instr,
								"Owned by players:" + shownames);
						...
						if (claimsshow.length == 1) { // only show specific
							...
							GriefPrevention
									.sendMessage(player, TextMode.Info,
											Messages.BlockClaimed,
											claim.getOwnerName());
							if (claim.allowEdit(player) == null) {
								GriefPrevention.sendMessage(player,
										TextMode.Info, "  " + claim.getWidth()
												+ "x" + claim.getHeight() + "="
												+ claim.getArea());
							}
							...
							if (!claim.isAdminClaim()
								...
								if (GriefPrevention.instance.getServer()
										.getPlayerExact(claim.getOwnerName()) == null)
									this.dataStore.clearCachedPlayerData(claim
											.getOwnerName());
							}
						}
					}
			...
			if (playerData.claimResizing == null) {
				...
				Claim checkclaim = GriefPrevention.instance.dataStore
						.getClaimAt(clickedBlock.getLocation(), true);
				...
				if (checkclaim != null) {
					...
					if (cannotedit == null
							&& GriefPrevention.instance.config_autosubclaims) {
			...
			if (playerData.claimResizing != null
				...
				newy2 = clickedBlock.getY()
						- wc.getClaimsExtendIntoGroundDistance();
				...
				if (playerData.claimResizing.parent == null) {
					...
					if (!playerData.claimResizing.isAdminClaim()
							&& (newWidth < wc.getMinClaimSize() || newHeight < wc
									.getMinClaimSize())) {
						GriefPrevention.sendMessage(player, TextMode.Err,
								Messages.ResizeClaimTooSmall,
								String.valueOf(wc.getMinClaimSize()));
						return;
					} else if (!playerData.claimResizing.isAdminClaim()
							&& newWidth * newHeight < wc
									.getMinClaimSizeBlocks()) {
						GriefPrevention.sendMessage(player, TextMode.Err,
								Messages.ResizeTooFewBlocks,
								String.valueOf(wc.getMinClaimSizeBlocks()));
						return;
					}
					...
					if (!playerData.claimResizing.isAdminClaim()
						...
						int blocksRemainingAfter = playerData
						...
						if (blocksRemainingAfter < 0) {
							...
							return;
																	...
																	// on the
																	// world...
							int worldblocksafter = playerData
									.getTotalClaimBlocksinWorld(player
											.getWorld())
									+ playerData.claimResizing.getArea()
									- newArea;
				...
				if (oldClaim.parent == null) {
					...
					Claim newClaim = new Claim(new Location(oldClaim
							.getLesserBoundaryCorner().getWorld(), newx1,
							newy1, newz1), new Location(oldClaim
							.getLesserBoundaryCorner().getWorld(), newx2,
							newy2, newz2), "", new String[] {},
					...
					if (!newClaim.contains(oldClaim.getLesserBoundaryCorner(),
							true, false)
							|| !newClaim.contains(
									oldClaim.getGreaterBoundaryCorner(), true,
									false)) {
						smaller = true;
				...
				CreateClaimResult result = GriefPrevention.instance.dataStore
						.resizeClaim(playerData.claimResizing, newx1, newx2,
								newy1, newy2, newz1, newz2, player);
				...
				if (result.succeeded == CreateClaimResult.Result.Success) {
					...
					GriefPrevention.sendMessage(player, TextMode.Success,
							Messages.ClaimResizeSuccess, String
									.valueOf(playerData
											.getRemainingClaimBlocks()));
					Visualization visualization = Visualization.FromClaim(
					...
					if (!playerData.claimResizing.getOwnerName().equals(
							playerName)) {
						GriefPrevention
								.AddLogEntry(playerName
										+ " resized "
										+ playerData.claimResizing
												.getOwnerName()
										+ "'s claim at "
										+ GriefPrevention
												.getfriendlyLocationString(playerData.claimResizing.lesserBoundaryCorner)
										+ ".");
					}
					...
					if (smaller
							&& wc.getAutoRestoreUnclaimed()
							&& GriefPrevention.instance
									.creativeRulesApply(oldClaim
											.getLesserBoundaryCorner())) {
						GriefPrevention.sendMessage(player, TextMode.Warn,
								Messages.UnclaimCleanupWarning);
						GriefPrevention.instance.restoreClaim(oldClaim,
								20L * 60 * 2); // 2 minutes
						GriefPrevention
								.AddLogEntry(player.getName()
										+ " shrank a claim @ "
										+ GriefPrevention
												.getfriendlyLocationString(playerData.claimResizing
														.getLesserBoundaryCorner()));
					}
					...
					// inform player
					if (playerData.claimResizing.parent != null
							&& playerData.claimResizing.parent == result.claim) {
						GriefPrevention.sendMessage(player, TextMode.Err,
			...
			if (lastShovelLocation == null) {
				...
				if (!GriefPrevention.instance.claimsEnabledForWorld(player
					...
					return;
						...
						&& !(player
								.hasPermission("griefprevention.ignoreclaimslimit"))) {

					// get the number of claims the player has in this world.
					if (wc.getClaimsPerPlayerLimit() <= playerData
							.getWorldClaims(clickedBlock.getWorld()).size()) {

						GriefPrevention.sendMessage(player, TextMode.Err,
								Messages.PlayerClaimLimit,
								String.valueOf(wc.getClaimsPerPlayerLimit()));
						return;
					}
				}
				...
				Visualization visualization = Visualization.FromClaim(
						new Claim(clickedBlock.getLocation(), clickedBlock
								.getLocation(), "", new String[] {},
								new String[] {}, new String[] {},
								new String[] {}, null, false), clickedBlock
								.getY(), VisualizationType.RestoreNature,
						player.getLocation());
				Visualization.Apply(player, visualization);
			...
			// other boundary corner
				...
				if (playerData.shovelMode != ShovelMode.Admin
						&& (newClaimWidth < wc.getMinClaimSize() || newClaimHeight < wc
								.getMinClaimSize())) {
					...
					GriefPrevention.sendMessage(player, TextMode.Err,
							...
							String.valueOf(wc.getMinClaimSize()));
					return;
				} else if (playerData.shovelMode != ShovelMode.Admin
						&& newClaimWidth * newClaimHeight < wc
								.getMinClaimSizeBlocks()) {
					GriefPrevention.sendMessage(player, TextMode.Err,
							Messages.ResizeTooFewBlocks,
							String.valueOf(wc.getMinClaimSizeBlocks()));
				...
				if (playerData.shovelMode != ShovelMode.Admin) {
					...
					if (wc.getClaims_maxBlocks() > 0) {
						...
						int currentworldclaim = playerData
								.getTotalClaimBlocksinWorld(player.getWorld());
						int diff = currentworldclaim + newClaimArea
								- wc.getClaims_maxBlocks();
						if (diff < 0) {
							GriefPrevention.sendMessage(player, TextMode.Err,
									Messages.InsufficientWorldBlocks,
									Math.abs(diff));
					...
					if (newClaimArea > remainingBlocks) {
						...
						if (player.isSneaking()
								&& newClaimArea - remainingBlocks > wc
										.getInsufficientSneakResetBound()
								&& wc.getInsufficientSneakResetBound() > 0) {
							GriefPrevention.sendMessage(player, TextMode.Instr,
									"First Point Abandoned!");
							playerData.lastShovelLocation = null;
							Visualization.Revert(player);
						} else {
				...
				CreateClaimResult result = this.dataStore.createClaim(
						...
						playerName, null, null, false, player, true);

				// if it didn't succeed, tell the player why
				if (result.succeeded == CreateClaimResult.Result.ClaimOverlap) {
					// if the claim it overlaps is owned by the player...

					if (result.claim.getOwnerName()
							.equalsIgnoreCase(playerName)) {
						// owned by the player.
						// make sure our larger claim entirely contains
						// the smaller one.

						if ((Claim.Contains(lastShovelLocation,
								clickedBlock.getLocation(),
								result.claim.getLesserBoundaryCorner(), true) && (Claim
								.Contains(
										lastShovelLocation,
										clickedBlock.getLocation(),
										result.claim.getGreaterBoundaryCorner(),
										true)))) {
							...
							result.claim.setLocation(lastShovelLocation,
									clickedBlock.getLocation());
							...
							GriefPrevention
									.sendMessage(player, TextMode.Success,
											...
											String.valueOf(playerData
													.getRemainingClaimBlocks()));
							// show message if world-based claim block limit is
							// in effect for this world.
							if (wc.getClaims_maxBlocks() > 0) {
								int worldblocksremaining = playerData
										.getRemainingClaimBlocks(player
												.getWorld());
								GriefPrevention.sendMessage(player,
										TextMode.Success,
										Messages.RemainingBlocksWorld,
										worldblocksremaining);
							}
				...
				// new claim
					...
					if (wc.getClaims_maxBlocks() > 0) {
						int worldblocksremaining = playerData
								.getRemainingClaimBlocks(player.getWorld());
						GriefPrevention.sendMessage(player, TextMode.Success,
								Messages.RemainingBlocksWorld,
								worldblocksremaining);
					}
					...
					if (wc.getClaimsPerPlayerLimit() > 0
							&& !player
									.hasPermission("griefprevention.ignoreclaimslimit")) {
						int numclaims = playerData.getWorldClaims(
								player.getWorld()).size();
						int remaining = wc.getClaimsPerPlayerLimit()
								- numclaims;
						System.out
								.println("Sending notification that Player has "
										+ remaining
										+ " Claims left in this world.");
						GriefPrevention.sendMessage(player, TextMode.Success,
								Messages.RemainingClaimsWorld,
