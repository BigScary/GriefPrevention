...
public class AbandonClaimCommand extends GriefPreventionCommand {
	private boolean abandonClaimHandler(Player player,
		...
		int costoverhead = (int) Math.floor(claimarea
				* (1 - wc.getClaimsAbandonReturnRatio()));
		// System.out.println("costoverhead:" + costoverhead);

		// verify ownership
		if (claim.allowEdit(player) != null) {
			GriefPrevention.sendMessage(player, TextMode.Err,
					Messages.NotYourClaim);
		}
		...
		// if auto-restoration is enabled,
				...
				&& wc.getClaimsAbandonNatureRestoration()) {
			GriefPrevention.sendMessage(player, TextMode.Warn,
					Messages.AbandonClaimRestoreWarning);
			playerData.warnedAboutMajorDeletion = true;
		} else if (!playerData.warnedAboutMajorDeletion
				&& costoverhead != claimarea) {
			playerData.warnedAboutMajorDeletion = true;
			GriefPrevention.sendMessage(player, TextMode.Warn,
					Messages.AbandonCostWarning, String.valueOf(costoverhead));
		}
		// if the claim has lots of surface water or some surface lava, warn the
		// player it will be cleaned up
		else if (!playerData.warnedAboutMajorDeletion
				&& claim.hasSurfaceFluids() && claim.parent == null) {
			GriefPrevention.sendMessage(player, TextMode.Warn,
			...
			if (costoverhead > 0) {
				...
				//
				GriefPrevention.sendMessage(player, TextMode.Warn,
						Messages.AbandonCost, 0, String.valueOf(costoverhead));
			}
			int remainingBlocks = playerData.getRemainingClaimBlocks();
			// tell the player how many claim blocks he has left
			GriefPrevention
					.sendMessage(player, TextMode.Success,
							Messages.AbandonSuccess, 0,
