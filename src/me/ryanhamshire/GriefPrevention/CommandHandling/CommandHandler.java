...
public class CommandHandler implements CommandExecutor {
	...
	public CommandHandler() {
		...
		for (GriefPreventionCommand iterate : GPCommands) {
			...
			if (gotlabels == null) {
				System.out.println("ERROR: GriefPrevention command named "
						+ iterate.getClass().getName()
						+ " returned null for labels!");
				continue;
			}
			for (String addcmd : gotlabels) {
				PluginCommand pc = GriefPrevention.instance.getCommand(addcmd);
				try {
					Debugger.Write("Attaching Command \"" + addcmd
							+ "\" to command class "
							+ iterate.getClass().getName(),
					...
					System.out
							.println("This is likely a mistake in plugin.yml, please notify the GriefPrevention maintainers :)");
					exx.printStackTrace();
				}
			}
	 ...
	 */
	static synchronized int transferClaimBlocks(String Source, String Target,
			int DesiredAmount) {
		...
		if (playerData != null && receiverData != null) {
			int xferamount = Math.min(playerData.accruedClaimBlocks,
					DesiredAmount);
