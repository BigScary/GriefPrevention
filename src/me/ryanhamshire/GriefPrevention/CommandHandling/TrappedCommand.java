...
public class TrappedCommand extends GriefPreventionCommand {
	...
	public boolean onCommand(CommandSender sender, Command command,
		...
		long nextTrappedUsage = lastTrappedUsage + 1000 * 60
				* wc.getClaimsTrappedCooldownMinutes();
		long now = new Date().getTime();
		if (now < nextTrappedUsage) {
			GriefPrevention.sendMessage(player, TextMode.Err,
					Messages.TrappedOnCooldown,
					String.valueOf(wc.getClaimsTrappedCooldownMinutes()),
		...
		PlayerRescueTask task = new PlayerRescueTask(player,
				player.getLocation());
		inst.getServer().getScheduler()
				.scheduleSyncDelayedTask(inst, task, 200L); // 20L ~ 1 second
