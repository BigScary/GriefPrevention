...
public class Visualization {
		...
		if (player.isOnline()) {
			GriefPrevention.instance
					...
					.scheduleSyncDelayedTask(
							GriefPrevention.instance,
							new VisualizationApplicationTask(player,
									playerData, visualization), 10L);
		}
	}
			...
			if (player.isOnline()) {
				for (Visualization visualization : playerData.ActiveVisualizations) {
					for (int i = 0; i < visualization.elements.size(); i++) {
						VisualizationElement element = visualization.elements
								.get(i);
	...
	public static Visualization FromClaim(Claim claim, int height,
			VisualizationType visualizationType, Location locality) {
		...
		for (int i = 0; i < claim.children.size(); i++) {
			visualization.addClaimElements(claim.children.get(i), height,
					VisualizationType.Subdivision, locality);
		}
	...
	private void addClaimElements(Claim claim, int height,
			VisualizationType visualizationType, Location locality) {
		...
		// bottom left corner
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				smallx, height, smallz), cornerMaterial, (byte) 0));
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				smallx + 1, height, smallz), accentMaterial, (byte) 0));
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				smallx, height, smallz + 1), accentMaterial, (byte) 0));

		// bottom right corner
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				bigx, height, smallz), cornerMaterial, (byte) 0));
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				bigx - 1, height, smallz), accentMaterial, (byte) 0));
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				bigx, height, smallz + 1), accentMaterial, (byte) 0));

		// top right corner
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				bigx, height, bigz), cornerMaterial, (byte) 0));
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				bigx - 1, height, bigz), accentMaterial, (byte) 0));
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				bigx, height, bigz - 1), accentMaterial, (byte) 0));

		// top left corner
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				smallx, height, bigz), cornerMaterial, (byte) 0));
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				smallx + 1, height, bigz), accentMaterial, (byte) 0));
		this.elements.add(new VisualizationElement(getVisibleLocation(world,
				smallx, height, bigz - 1), accentMaterial, (byte) 0));
		...
		for (int x = smallx + 10; x < bigx - 10; x += 10) {
			if (x > minx && x < maxx)
				this.elements.add(new VisualizationElement(getVisibleLocation(
						world, x, height, bigz), accentMaterial, (byte) 0));
		}
		...
		for (int x = smallx + 10; x < bigx - 10; x += 10) {
			if (x > minx && x < maxx)
				this.elements.add(new VisualizationElement(getVisibleLocation(
						world, x, height, smallz), accentMaterial, (byte) 0));
		}
		...
		for (int z = smallz + 10; z < bigz - 10; z += 10) {
			if (z > minz && z < maxz)
				this.elements.add(new VisualizationElement(getVisibleLocation(
						world, smallx, height, z), accentMaterial, (byte) 0));
		}
		...
		for (int z = smallz + 10; z < bigz - 10; z += 10) {
			if (z > minz && z < maxz)
				this.elements.add(new VisualizationElement(getVisibleLocation(
						world, bigx, height, z), accentMaterial, (byte) 0));
		}
	}
	...
	private static Location getVisibleLocation(World world, int x, int y, int z) {
		...
		BlockFace direction = (isTransparent(block)) ? BlockFace.DOWN
				: BlockFace.UP;
	...
	private static boolean isTransparent(Block block) {
		WorldConfig applicableWorld = GriefPrevention.instance
				.getWorldCfg(block.getWorld());
		if (applicableWorld.getModsContainerTrustIds() != null
				&& applicableWorld.getModsContainerTrustIds().contains(
						block.getType()))
			return true;
		if (applicableWorld.getModsAccessTrustIds() != null
				&& applicableWorld.getModsAccessTrustIds().contains(
						block.getType()))
			return true;
		return (block.getType() == Material.AIR
				...
				|| block.getType() == Material.CHEST || block.getType() == Material.ENDER_CHEST);
	}
}
