...
public abstract class DataStore {
	 ...
	 */
	public final static String dataLayerFolderPath = "plugins" + File.separator
			+ "GriefPrevention";
	/**
	 * Old location of the dataLayer folder This will be the GriefPreventionData
	 * folder within the bukkit plugins/ folder.
	 */
	public final static String oldDataLayerFolderPath = "plugins"
			+ File.separator + "GriefPreventionData";

	public final static String configFilePath = dataLayerFolderPath
			+ File.separator + "config.yml";
	final static String messagesFilePath = dataLayerFolderPath + File.separator
			+ "messages.yml";
	...
	public synchronized int getGroupBonusBlocks(String playerName) {
		...
		while (iterator.hasNext()) {
			...
			Player player = GriefPrevention.instance.getServer().getPlayer(
					playerName);
	 ...
	 * @throws Exception
	 */
	synchronized public void changeClaimOwner(Claim claim, String newOwnerName)
			throws Exception {
	...
	synchronized void addClaim(Claim newClaim) {
		...
		if (newClaim.parent != null) {
			if (newClaim.subClaimid == null) {
				GriefPrevention.AddLogEntry("Setting Subclaim ID to:"
						+ String.valueOf(1 + newClaim.parent.children.size()));
	...
	String locationToString(Location location) {
		StringBuilder stringBuilder = new StringBuilder(location.getWorld()
				.getName());
	...
	public Location locationFromString(String string) throws Exception {
		...
		if (elements.length != 4) {
			throw new Exception(
					"Expected four distinct parts to the location string:{"
							+ string + "}");
		}
	...
	void WorldUnloaded(World worldunload) {
		Debugger.Write("World " + worldunload + " is unloading.",
				DebugLevel.Informational);
		int accum = 0;
		try {
			for (Claim c : this.getClaimArray().claimworldmap.get(worldunload
					.getName())) {
				this.saveClaim(c);
		...
		Debugger.Write("Saved and removed " + accum + " Claims.",
				DebugLevel.Informational);
	}
	...
	// deletes a claim or subdivision
	synchronized private boolean deleteClaim(Claim claim, boolean sendevent,
			Player p) {
	...
	protected void ForceLoadAllClaims() {
		...
		for (String iterate : GriefPrevention.instance.Configuration
				.GetWorldConfigNames()) {
			GriefPrevention.AddLogEntry("Force-loading World:" + iterate);
			World loadedworld = Bukkit.getServer().createWorld(
					new WorldCreator(iterate));
	...
	synchronized public ClaimDistanceResult getNearestClaim(
			Location testLocation, int MaxDistance) {
		...
		for (int sign : signsuse) {
			...
			for (int xtest = 0; xtest < MaxDistance; xtest++) {
				...
				for (int ztest = 0; ztest < MaxDistance; ztest++) {
					...
					Location corelocation = new Location(
							testLocation.getWorld(), useXPos, 64, useZPos);
					Claim grabclaim = getClaimAt(corelocation, true);
					if (grabclaim != null) {
						return new ClaimDistanceResult(grabclaim, Math.min(
								xtest, ztest));
					}
	...
	synchronized public Claim getClaimAt(Location location,
			boolean ignoreHeight, Claim cachedClaim) {

		if (cachedClaim != null && cachedClaim.inDataStore
				&& cachedClaim.contains(location, ignoreHeight, false))
			return cachedClaim;
	...
	synchronized public Set<Claim> getClaimsIn(Location Lesser,
			Location Greater, boolean Inclusive) {
		...
		for (int x = LessX; x < GreatX; x += minclaimsize) {
			for (int z = LessZ; z < GreatZ; z += minclaimsize) {
				...
				Claim foundclaim = getClaimAt(new Location(Lesser.getWorld(),
						x, 64, z), false);
				if (foundclaim != null) {
	...
	synchronized public CreateClaimResult createClaim(World world, int x1,
			int x2, int y1, int y2, int z1, int z2, String ownerName,
			Claim parent, Long id, boolean neverdelete, Player creator) {
		return createClaim(world, x1, x2, y1, y2, z1, z2, ownerName, parent,
				id, neverdelete, creator, true);
	}
	...
	synchronized public CreateClaimResult createClaim(World world, int x1,
			int x2, int y1, int y2, int z1, int z2, String ownerName,
			Claim parent, Long id, boolean neverdelete, Player creator,
			boolean doRaiseEvent) {
		return createClaim(world, x1, x2, y1, y2, z1, z2, ownerName, parent,
				id, false, null, creator, doRaiseEvent);
	}

	@Deprecated
	synchronized public CreateClaimResult createClaim(World world, int x1,
			int x2, int y1, int y2, int z1, int z2, String ownerName,
			Claim parent, Long id) {
		return createClaim(world, x1, x2, y1, y2, z1, z2, ownerName, parent,
				id, false, null);
	}
	...
	synchronized private CreateClaimResult createClaim(World world, int x1,
			int x2, int y1, int y2, int z1, int z2, String ownerName,
			Claim parent, Long id, boolean neverdelete, Claim oldclaim,
			Player claimcreator, boolean doRaiseEvent) {
		...
		Claim newClaim = new Claim(new Location(world, smallx, smally, smallz),
				new Location(world, bigx, bigy, bigz), ownerName,
		...
		if (oldclaim == null) {
			if (doRaiseEvent) {
				ClaimBeforeCreateEvent claimevent = new ClaimBeforeCreateEvent(
						newClaim, claimcreator);
		...
		ClaimAfterCreateEvent claimevent = new ClaimAfterCreateEvent(newClaim,
				claimcreator);
		Bukkit.getServer().getPluginManager().callEvent(claimevent);
	...
	synchronized public void extendClaim(Claim claim, int newDepth) {
		...
		WorldConfig wc = GriefPrevention.instance.getWorldCfg(claim
				.getLesserBoundaryCorner().getWorld());
	 ...
	 * @see #onCooldown()
	 */
	synchronized public boolean startSiege(Player attacker, Player defender,
			Claim defenderClaim) {
		...
		siegeData.checkupTaskID = GriefPrevention.instance
				.getServer()
				.getScheduler()
	 ...
	 */
	synchronized public void endSiege(SiegeData siegeData, String winnerName,
			String loserName, boolean death) {
		...
		this.siegeCooldownRemaining.put(siegeData.attacker.getName() + "_"
				+ siegeData.defender.getName(), cooldownEnd);
		...
		if (!siegeData.SiegedBlocks.isEmpty()) {
			...
			GriefPrevention.AddLogEntry("reverted " + revertedCount
					+ " Sieged Blocks.");
		...
		for (int i = 0; i < siegeData.claims.size(); i++) {
			...
			this.siegeCooldownRemaining.put(siegeData.attacker.getName() + "_"
					+ claim.getOwnerName(), cooldownEnd);
		...
		GriefPrevention.instance.getServer().getScheduler()
				.cancelTask(siegeData.checkupTaskID);
		...
		if (grantAccess) {
			Player winner = GriefPrevention.instance.getServer().getPlayer(
					winnerName);
			if (winner != null) {
				// notify the winner
				GriefPrevention.sendMessage(winner, TextMode.Success,
		...
		if (death) {
			Player winner = GriefPrevention.instance.getServer().getPlayer(
					winnerName);
			Player loser = GriefPrevention.instance.getServer().getPlayer(
					loserName);
			if (winner != null && loser != null) {
				WorldConfig wc = GriefPrevention.instance.getWorldCfg(winner
						.getWorld());
				...
				for (int j = 0; j < loserItems.length; j++) {
					if (loserItems[j] == null
							|| loserItems[j].getType() == Material.AIR
							|| loserItems[j].getAmount() == 0)
						continue;

					HashMap<Integer, ItemStack> wontFitItems = winner
							.getInventory().addItem(loserItems[j]);
					...
					for (Object key : keys) {
						winnerLocation.getWorld().dropItemNaturally(
								winnerLocation, wontFitItems.get(key));
					}
				}
			}
		}
	 ...
	 * @return
	 */
	synchronized public boolean onCooldown(Player attacker, Player defender,
			Claim defenderClaim) {
		Long cooldownEnd = null;

		// look for an attacker/defender cooldown
		if (this.siegeCooldownRemaining.get(attacker.getName() + "_"
				+ defender.getName()) != null) {
			cooldownEnd = this.siegeCooldownRemaining.get(attacker.getName()
					+ "_" + defender.getName());
			...
			this.siegeCooldownRemaining.remove(attacker.getName() + "_"
					+ defender.getName());
		}

		// look for an attacker/claim cooldown
		if (cooldownEnd == null
				&& this.siegeCooldownRemaining.get(attacker.getName() + "_"
						+ defenderClaim.getOwnerName()) != null) {
			cooldownEnd = this.siegeCooldownRemaining.get(attacker.getName()
					+ "_" + defenderClaim.getOwnerName());
	...
	@Deprecated
	synchronized public void deleteClaimsForPlayer(String playerName,
			boolean deleteCreativeClaims) {
	 ...
	 */
	synchronized public void deleteClaimsForPlayer(String playerName,
			boolean deleteCreativeClaims, boolean deleteLockedClaims) {
	...
	synchronized public CreateClaimResult resizeClaim(Claim claim, Location p1,
			Location p2, Player resizer) {
	 ...
	 * @return
	 */
	synchronized public CreateClaimResult resizeClaim(Claim claim, int newx1,
			int newx2, int newy1, int newy2, int newz1, int newz2,
			Player claimcreator) {

		Location newLesser = new Location(claim.getLesserBoundaryCorner()
				.getWorld(), newx1, newy1, newz1);
		Location newGreater = new Location(claim.getLesserBoundaryCorner()
				.getWorld(), newx2, newy2, newz2);
		if (claim.parent != null) {
			// make sure both borders are within our parent claim.
			if (!(claim.parent.contains(newLesser, true, false) || claim
					.contains(newGreater, true, false))) {
		...
		ClaimResizeEvent cre = new ClaimResizeEvent(claim, newLesser,
				newGreater, claimcreator);
		...
		// overlap
		CreateClaimResult result = this.createClaim(claim
				.getLesserBoundaryCorner().getWorld(), newx1, newx2, newy1,
				newy2, newz1, newz2, claim.getOwnerName(), claim.parent,
				claim.id, claim.neverdelete, claim, claimcreator, false);
		...
		if (result.succeeded == CreateClaimResult.Result.Success) {
			...
			for (int i = 0; i < builders.size(); i++)
				result.claim.setPermission(builders.get(i),
						ClaimPermission.Build);

			for (int i = 0; i < containers.size(); i++)
				result.claim.setPermission(containers.get(i),
						ClaimPermission.Inventory);

			for (int i = 0; i < accessors.size(); i++)
				result.claim.setPermission(accessors.get(i),
	...
	private void loadMessages() {
		...
		// initialize defaults
		this.addDefault(defaults, Messages.RespectingClaims,
				"Now respecting claims.", null);
		this.addDefault(defaults, Messages.IgnoringClaims,
				"Now ignoring claims.", null);
		this.addDefault(
				defaults,
		...
		this.addDefault(
				...
				"All claims abandoned except for locked claims.  You now have {0} available claim blocks.",
				"0: remaining blocks");
		this.addDefault(
				defaults,
				Messages.SuccessfulAbandonIncludingLocked,
				"All claims abandoned including locked claims.  You now have {0} available claim blocks.",
				"0: remaining blocks");
		this.addDefault(
				defaults,
		...
		this.addDefault(defaults, Messages.TransferClaimAdminOnly,
				...
				null);
		this.addDefault(defaults, Messages.PlayerNotFound, "Player not found.",
				null);
		this.addDefault(
				defaults,
				...
				null);
		this.addDefault(defaults, Messages.TransferSuccess,
				"Claim transferred.", null);
		this.addDefault(defaults, Messages.TrustListNoClaim,
				"Stand inside the claim you're curious about.", null);
		this.addDefault(defaults, Messages.ClearPermsOwnerOnly,
				"Only the claim owner can clear all permissions.", null);
		this.addDefault(
				defaults,
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.NoOwnerTrust,
				"Only {0} Can do that here.", "0:Owner of the claim");
		...
		this.addDefault(
				...
				"0: untrusted player");
		this.addDefault(defaults, Messages.OnlySellBlocks,
				"Claim blocks may only be sold, not purchased.", null);
		this.addDefault(defaults, Messages.BlockPurchaseCost,
		...
		this.addDefault(defaults, Messages.OnlyPurchaseBlocks,
				"Claim blocks may only be purchased, not sold.", null);
		this.addDefault(
				defaults,
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.BasicClaimsMode,
				"Returned to basic claim creation mode.", null);
		this.addDefault(
				defaults,
				...
				null);
		this.addDefault(defaults, Messages.SubdivisionDemo,
				"Land Claim Help:  http://tinyurl.com/7urdtue", null);
		this.addDefault(defaults, Messages.DeleteClaimMissing,
				"There's no claim here.", null);
		this.addDefault(
				defaults,
		...
		this.addDefault(
				...
				"This claim is locked.  If you're sure you want to delete it, use /DeleteClaim again.",
				null);
		this.addDefault(defaults, Messages.DeleteSuccess, "Claim deleted.",
				null);
		this.addDefault(defaults, Messages.CantDeleteAdminClaim,
				"You don't have permission to delete administrative claims.",
				null);
		this.addDefault(defaults, Messages.DeleteAllSuccessExcludingLocked,
				"Deleted all of {0}'s claims excluding locked claims.",
				"0: owner's name");
		this.addDefault(defaults, Messages.DeleteAllSuccessIncludingLocked,
				"Deleted all of {0}'s claims including locked claims.",
				"0: owner's name");
		this.addDefault(defaults, Messages.NoDeletePermission,
				"You don't have permission to delete claims.", null);
		this.addDefault(defaults, Messages.AllAdminDeleted,
				"Deleted all administrative claims.", null);
		this.addDefault(
				defaults,
		...
		this.addDefault(defaults, Messages.NotTrappedHere,
				"You can build here.  Save yourself.", null);
		this.addDefault(
				defaults,
				Messages.TrappedOnCooldown,
				"You used /trapped within the last {0} Minutes.  You have to wait about {1} more minutes before using it again.",
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.NonSiegeWorld,
				"Siege is disabled here.", null);
		this.addDefault(defaults, Messages.AlreadySieging,
				"You're already involved in a siege.", null);
		this.addDefault(defaults, Messages.AlreadyUnderSiegePlayer,
				"{0} is already under siege.  Join the party!",
				"0: defending player");
		this.addDefault(defaults, Messages.NotSiegableThere,
				"{0} isn't protected there.", "0: defending player");
		this.addDefault(defaults, Messages.SiegeTooFarAway,
				"You're too far away to siege.", null);
		this.addDefault(defaults, Messages.NoSiegeDefenseless,
				"That player is defenseless.  Go pick on somebody else.", null);
		this.addDefault(defaults, Messages.AlreadyUnderSiegeArea,
				"That area is already under siege.  Join the party!", null);
		this.addDefault(defaults, Messages.NoSiegeAdminClaim,
				"Siege is disabled in this area.", null);
		this.addDefault(
				defaults,
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.NotYourClaim,
				"This isn't your claim.", null);
		this.addDefault(
				defaults,
				...
				null);
		this.addDefault(defaults, Messages.AbandonSuccess,
		...
		this.addDefault(defaults, Messages.GrantPermissionNoClaim,
				...
				null);
		this.addDefault(defaults, Messages.GrantPermissionConfirmation,
		...
		this.addDefault(defaults, Messages.ContainersPermission,
				"access containers and animals", null);
		this.addDefault(defaults, Messages.AccessPermission,
				"use buttons and levers", null);
		this.addDefault(defaults, Messages.PermissionsPermission,
				"manage permissions", null);
		this.addDefault(defaults, Messages.LocationCurrentClaim,
				"in this claim", null);
		this.addDefault(defaults, Messages.LocationAllClaims,
				"in all your claims", null);
		this.addDefault(
				defaults,
				Messages.PvPImmunityStart,
				"You're protected from attack by other players as long as your inventory is empty.",
				null);
		this.addDefault(defaults, Messages.SiegeNoDrop,
				"You can't give away items while involved in a siege.", null);
		this.addDefault(
				defaults,
				Messages.DonateItemsInstruction,
				"To give away the item(s) in your hand, left-click the chest again.",
				null);
		this.addDefault(defaults, Messages.ChestFull, "This chest is full.",
				null);
		this.addDefault(defaults, Messages.DonationSuccess,
				"Item(s) transferred to chest!", null);
		this.addDefault(defaults, Messages.PlayerTooCloseForFire,
				"You can't start a fire this close to {0}.",
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.ChestClaimConfirmation,
				"This chest is protected.", null);
		this.addDefault(
				defaults,
				...
				null);
		this.addDefault(defaults, Messages.TrustCommandAdvertisement,
				"Use the /trust command to grant other players access.", null);
		this.addDefault(
				defaults,
				Messages.GoldenShovelAdvertisement,
				"To claim more land, you need a golden shovel.  When you equip one, you'll get more information.",
				null);
		this.addDefault(
				defaults,
				...
				null);
		this.addDefault(defaults, Messages.ThatPlayerPvPImmune,
				"You can't injure defenseless players.", null);
		this.addDefault(defaults, Messages.CantFightWhileImmune,
				"You can't fight someone while you're protected from PvP.",
				null);
		this.addDefault(defaults, Messages.NoDamageClaimedEntity,
				"That belongs to {0}.", "0: owner name");
		this.addDefault(defaults, Messages.ShovelBasicClaimMode,
				"Shovel returned to basic claims mode.", null);
		this.addDefault(defaults, Messages.RemainingBlocks,
				"You may claim up to {0} more blocks.", "0: remaining blocks");
		this.addDefault(
				defaults,
				Messages.InsufficientWorldBlocks,
				"You've used too many claim blocks in this world. You need to free up {0} to make that claim.",
				"0: required claim blocks");
		this.addDefault(defaults, Messages.RemainingBlocksWorld,
				"You may claim up to {0} more blocks in this world.",
				"0: remaining blocks in world");

		this.addDefault(defaults, Messages.RemainingClaimsWorld,
				"You may make {0} more claims in this world.",
				"0: remaining claims in world.");
		this.addDefault(defaults, Messages.CreativeBasicsDemoAdvertisement,
				"Land Claim Help:  http://tinyurl.com/c7bajb8", null);
		this.addDefault(defaults, Messages.SurvivalBasicsDemoAdvertisement,
				"Land Claim Help:  http://tinyurl.com/6nkwegj", null);
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.PvPNoDrop,
				"You can't drop items while in PvP combat.", null);
		this.addDefault(defaults, Messages.SiegeNoTeleport,
				"You can't teleport out of a besieged area.", null);
		this.addDefault(defaults, Messages.BesiegedNoTeleport,
				"You can't teleport into a besieged area.", null);
		this.addDefault(defaults, Messages.SiegeNoContainers,
				"You can't access containers while involved in a siege.", null);
		this.addDefault(defaults, Messages.PvPNoContainers,
				"You can't access containers during PvP combat.", null);
		this.addDefault(defaults, Messages.PvPImmunityEnd,
				"Now you can fight with other players.", null);
		this.addDefault(defaults, Messages.NoBedPermission,
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.NoLavaNearOtherPlayer,
				"You can't place lava this close to {0}.", "0: nearby player");
		this.addDefault(defaults, Messages.TooFarAway, "That's too far away.",
				null);
		this.addDefault(defaults, Messages.BlockNotClaimed,
				"No one has claimed this block.", null);
		this.addDefault(defaults, Messages.BlockClaimed,
				"That block has been claimed by {0}.", "0: claim owner");
		this.addDefault(defaults, Messages.SiegeNoShovel,
				"You can't use your shovel tool while involved in a siege.",
				null);
		this.addDefault(defaults, Messages.RestoreNaturePlayerInChunk,
				"Unable to restore.  {0} is in that chunk.", "0: nearby player");
		this.addDefault(defaults, Messages.NoCreateClaimPermission,
				"You don't have permission to claim land.", null);
		this.addDefault(
				defaults,
				Messages.ResizeClaimTooSmall,
				"This new size would be too small.  Claims must be at least {0} x {0}.",
				"0: minimum claim size");
		this.addDefault(
				defaults,
				Messages.ResizeNeedMoreBlocks,
				"You don't have enough blocks for this size.  You need {0} more.",
				"0: how many needed");
		this.addDefault(defaults, Messages.ClaimResizeSuccess,
				"Claim resized.  You now have {0} available claim blocks.",
				"0: remaining blocks");
		this.addDefault(
				defaults,
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.CreateSubdivisionOverlap,
				"Your selected area overlaps another subdivision.", null);
		...
		this.addDefault(
				...
				"0: other claim owner");
		this.addDefault(defaults, Messages.ClaimsDisabledWorld,
				"Land claims are disabled in this world.", null);
		this.addDefault(
				defaults,
				Messages.ClaimStart,
		...
		this.addDefault(
				defaults,
				Messages.NewClaimTooSmall,
				"This claim would be too small.  Any claim must be at least {0} x {0}.",
				"0: minimum claim size");
		this.addDefault(
				defaults,
		...
		this.addDefault(defaults, Messages.CreateClaimSuccess,
				"Claim created!  Use /trust to share it with friends.", null);
		this.addDefault(
				defaults,
				Messages.SiegeWinDoorsOpen,
				"Congratulations!  Buttons and levers are temporarily unlocked (five minutes).",
				null);
		this.addDefault(defaults, Messages.RescueAbortedMoved,
				"You moved!  Rescue cancelled.", null);
		this.addDefault(defaults, Messages.SiegeDoorsLockedEjection,
				"Looting time is up!  Ejected from the claim.", null);
		this.addDefault(defaults, Messages.NoModifyDuringSiege,
				"Claims can't be modified while under siege.", null);
		this.addDefault(defaults, Messages.OnlyOwnersModifyClaims,
				"Only {0} can modify this claim.", "0: owner name");
		this.addDefault(defaults, Messages.NoBuildUnderSiege,
				"This claim is under siege by {0}.  No one can build here.",
				"0: attacker name");
		this.addDefault(defaults, Messages.NoBuildPvP,
				"You can't build in claims during PvP combat.", null);
		this.addDefault(defaults, Messages.NoBuildPermission,
				"You don't have {0}'s permission to build here.",
				"0: owner name");
		this.addDefault(defaults, Messages.NonSiegeMaterial,
				"That material is too tough to break.", null);
		this.addDefault(defaults, Messages.NoOwnerBuildUnderSiege,
				"You can't make changes while under siege.", null);
		this.addDefault(defaults, Messages.NoAccessPermission,
		...
		this.addDefault(
				...
				"0: attacker name");
		this.addDefault(defaults, Messages.NoContainersPermission,
				"You don't have {0}'s permission to use that.",
				"0: owner's name.  containers also include crafting blocks");
		this.addDefault(defaults, Messages.OwnerNameForAdminClaims,
				"an administrator",
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.YouHaveNoClaims,
				"You don't have any land claims.", null);
		this.addDefault(
				defaults,
				Messages.ConfirmFluidRemoval,
				"Abandoning this claim will remove all your lava and water.  If you're sure, use /AbandonClaim again.",
				null);
		this.addDefault(
				defaults,
				Messages.ConfirmAbandonLockedClaim,
				"This claim has been locked.  If you really want to abandon it, use /AbandonClaim again.",
				null);
		this.addDefault(defaults, Messages.AutoBanNotify,
				"Auto-banned {0}({1}).  See logs for details.", null);
		this.addDefault(
				defaults,
		...
		this.addDefault(defaults, Messages.InvalidPermissionID,
				...
				null);
		this.addDefault(defaults, Messages.UntrustOwnerOnly,
				"Only {0} can revoke permissions here.",
				"0: claim owner's name");
		this.addDefault(
				defaults,
		...
		this.addDefault(defaults, Messages.PlayerOfflineTime,
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.CommandBannedInPvP,
				"You can't use that command while in PvP combat.", null);
		this.addDefault(
				defaults,
				...
				null);
		this.addDefault(defaults, Messages.BuySellNotConfigured,
				"Sorry, buying and selling claim blocks is disabled.", null);
		this.addDefault(defaults, Messages.NoTeleportPvPCombat,
				"You can't teleport while fighting another player.", null);
		this.addDefault(defaults, Messages.NoTNTDamageAboveSeaLevel,
				"Warning: TNT will not destroy blocks above sea level.", null);
		this.addDefault(defaults, Messages.NoTNTDamageClaims,
				"Warning: TNT will not destroy claimed blocks.", null);
		this.addDefault(defaults, Messages.IgnoreClaimsAdvertisement,
				"To override, use /IgnoreClaims.", null);
		this.addDefault(defaults, Messages.NoPermissionForCommand,
		...
		this.addDefault(
				...
				null);
		this.addDefault(defaults, Messages.PlayerInPvPSafeZone,
				"That player is in a PvP safe zone.", null);
		this.addDefault(
				defaults,
				Messages.ClaimLocked,
				"This claim has been successfully locked against accidental/automatic deletion. Use /unlockclaim to unlock.",
				null);
		this.addDefault(defaults, Messages.ClaimUnlocked,
				"This claim has been successfully unlocked.", null);
		// this.addDefault(defaults, Messages.LoginSpamWait,
		// "You must wait about {0} more minutes before logging-in again.",
		// null);
		this.addDefault(defaults, Messages.LoginSpamWaitSeconds,
				"You must wait {0} more seconds before logging-in again.", null);
		this.addDefault(
				defaults,
				Messages.AbandonClaimRestoreWarning,
				"Abandoning this claim will restore nature! If you still want to abandon it, use /abandonclaim again.",
				null);
		this.addDefault(defaults, Messages.AbandonCost,
				"you lose {0} Claim blocks from abandoning this claim.",
				"0:Number of claim blocks lost");
		this.addDefault(
				defaults,
				Messages.AbandonCostWarning,
				"You will lose {0} Claim blocks if you abandon this claim. enter /abandonclaim again to confirm.",
				"0:Number of claim blocks that will be lost");
		this.addDefault(defaults, Messages.NoVillagerTradeOutsideClaims,
				"You cannot trade with Villagers outside of Claims.", null);
		this.addDefault(
				defaults,
				Messages.PlayerClaimLimit,
				"You have reached the claim limit ({0}) and cannot create new claims.",
				"0:Configuration Claim Limit.");
		this.addDefault(defaults, Messages.ClearManagersNotFound,
				"You must be in a claim to clear it's Managers.", null);
		this.addDefault(defaults, Messages.ClearManagersSuccess,
				"Cleared all Managers in this Claim.", null);
		this.addDefault(defaults, Messages.ClearManagersNotOwned,
				"Only {0} can clear managers in their Claim.", "0:Claim Owner");
		this.addDefault(defaults, Messages.ClearManagersNotAdmin,
				"Only Administrators can change managers on an admin claim.",
				null);
		this.addDefault(defaults, Messages.GroupNotFound,
				"Group {0} Not found.", "0:Name of Group");
		this.addDefault(defaults, Messages.ConfigDisabled,
				"{0} has been disabled for this location.",
				"0:name of operation");
		this.addDefault(defaults, Messages.TamedDeathDefend,
				"A {0} has been killed in defense of your Claim!",
				"0:Type of tamed animal;1:Attacker");
		this.addDefault(
				defaults,
				Messages.NoGiveClaimsPermission,
				"You do not have Permission to Give your Claims to other players",
				null);
		this.addDefault(defaults, Messages.NoAdminClaimsPermission,
				"You need Administrator Claims Permission to do that.", null);
		this.addDefault(defaults, Messages.GiveSuccessSender,
				"Claim Owner changed from {0} to {1} Successfully.",
				"0:Sender;1:Recipient");
		this.addDefault(defaults, Messages.GiveSuccessTarget,
				"{0} has transferred a Claim to you.", "0:Sender");
		this.addDefault(
				defaults,
				Messages.ResizeFailOutsideParent,
				"Cannot resize subdivision as it would extend outside the containing claim.",
				null);
		this.addDefault(
				defaults,
				Messages.BlockPlacementTooClose,
				"You cannot place this within {0} Blocks of an existing claim.",
				"0:Number of max blocks");
		this.addDefault(defaults, Messages.CantSiegeYourself,
				"You cannot lay siege on yourself!", null);
		this.addDefault(defaults, Messages.AutoSubClaimsEnter,
				"Switching to Subdivide Claim Mode", null);
		this.addDefault(defaults, Messages.AutoSubClaimsExit,
				"Switching to Standard Claim Mode", null);
		this.addDefault(defaults, Messages.AutoSubClaimsNoPermission,
				"You need permission in a claim to make subdivisions.", null);
		this.addDefault(defaults, Messages.CreateClaimTooFewBlocks,
				"That claim would require too many blocks. You need {0} more.",
				"0:Numberof minimum claim blocks");
		this.addDefault(
				defaults,
				Messages.ResizeNeedMoreBlocks,
				"The resized claim would be too large. You need {0} more claim blocks.",
				"0:number of claim blocks required");
		this.addDefault(
				defaults,
				Messages.ResizeTooFewBlocks,
				"That claim would not take up enough space. Claims must use at least {0} Blocks.",
				"0:Minimum blocks in a claim");
		// load the config file
		FileConfiguration config = YamlConfiguration
				.loadConfiguration(new File(messagesFilePath));
		...
		for (int i = 0; i < messageIDs.length; i++) {
			...
			if (messageData == null) {
				GriefPrevention
						.AddLogEntry("Missing message for " + messageID.name()
								+ ".  Please contact the developer.");
				messageData = new CustomizableMessage(messageID,
						"Missing message!  ID: " + messageID.name()
								+ ".  Please contact a server admin.", null);
			}

			// read the message from the file, use default if necessary
			this.messages[messageID.ordinal()] = config.getString("Messages."
					+ messageID.name() + ".Text", messageData.text);
			config.set("Messages." + messageID.name() + ".Text",
					this.messages[messageID.ordinal()]);

			if (messageData.notes != null) {
				messageData.notes = config.getString(
						"Messages." + messageID.name() + ".Notes",
						messageData.notes);
				config.set("Messages." + messageID.name() + ".Notes",
						messageData.notes);
			}
		}
		...
		try {
			...
			GriefPrevention
					.AddLogEntry("Unable to write to the configuration file at \""
							+ DataStore.messagesFilePath + "\"");
		}
	...
	synchronized public Long[] getClaimIds() {
		return claims.claimmap.keySet().toArray(
				new Long[claims.claimmap.size()]);
	}
