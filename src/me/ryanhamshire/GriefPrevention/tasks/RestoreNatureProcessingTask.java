...
public class RestoreNatureProcessingTask implements Runnable {
	...
	private ArrayList<Integer> notAllowedToHang; // natural blocks which don't
													// naturally hang in their
													// air
	private ArrayList<Integer> playerBlocks; // a "complete" list of
												// player-placed blocks. MUST BE
												// MAINTAINED as patches
												// introduce more

	public RestoreNatureProcessingTask(BlockSnapshot[][][] snapshots, int miny,
			Environment environment, Biome biome,
			Location lesserBoundaryCorner, Location greaterBoundaryCorner,
			int seaLevel, boolean aggressiveMode, boolean creativeMode,
			Player player) {
		...
		this.playerBlocks.addAll(RestoreNatureProcessingTask.getPlayerBlocks(
				this.environment, this.biome));
	...
	public void run() {
		...
		RestoreNatureExecutionTask task = new RestoreNatureExecutionTask(
				this.snapshots, this.miny, this.lesserBoundaryCorner,
				this.greaterBoundaryCorner, this.player);
	...
	private void fillBigHoles() {
		for (int x = 1; x < snapshots.length - 1; x++) {
			for (int z = 1; z < snapshots[0][0].length - 1; z++) {
				...
				if (this.snapshots[x][this.seaLevel - 2][z].typeId == Material.AIR
						.getId()
						|| this.snapshots[x][this.seaLevel - 2][z].typeId == Material.LAVA
								.getId()
						|| (this.snapshots[x][this.seaLevel - 2][z].typeId == Material.WATER
								.getId() || this.snapshots[x][this.seaLevel - 2][z].data != 0)) {
					this.snapshots[x][this.seaLevel - 2][z].typeId = Material.STONE
							.getId();
				}
				...
				// falls down)
				if (this.snapshots[x][this.seaLevel - 3][z].typeId == Material.AIR
						.getId()
						|| this.snapshots[x][this.seaLevel - 3][z].typeId == Material.LAVA
								.getId()
						|| (this.snapshots[x][this.seaLevel - 3][z].typeId == Material.WATER
								.getId() || this.snapshots[x][this.seaLevel - 3][z].data != 0)) {
					this.snapshots[x][this.seaLevel - 3][z].typeId = Material.STONE
							.getId();
				}
			}
		}
		...
		for (int x = 1; x < snapshots.length - 1; x++) {
			for (int z = 1; z < snapshots[0][0].length - 1; z++) {
				for (int y = snapshots[0].length - 2; y > miny; y--) {
					...
					if (aboveBlock.typeId == Material.SAND.getId()
	...
	private void reduceStone() {
		for (int x = 1; x < snapshots.length - 1; x++) {
			for (int z = 1; z < snapshots[0][0].length - 1; z++) {
				...
				while (thisy > this.seaLevel - 1
						&& (this.snapshots[x][thisy][z].typeId == Material.STONE
								.getId() || this.snapshots[x][thisy][z].typeId == Material.SANDSTONE
								.getId())) {
					...
					if (leftBlock.typeId != Material.AIR.getId()
							&& leftBlock.typeId != Material.LEAVES.getId()
							&& leftBlock.typeId != Material.VINE.getId()) {
						adjacentBlockCount++;
					}
					if (rightBlock.typeId != Material.AIR.getId()
							&& rightBlock.typeId != Material.LEAVES.getId()
							&& rightBlock.typeId != Material.VINE.getId()) {
						adjacentBlockCount++;
					}
					if (downBlock.typeId != Material.AIR.getId()
							&& downBlock.typeId != Material.LEAVES.getId()
							&& downBlock.typeId != Material.VINE.getId()) {
						adjacentBlockCount++;
					}
					if (upBlock.typeId != Material.AIR.getId()
							&& upBlock.typeId != Material.LEAVES.getId()
		...
		for (int x = 1; x < snapshots.length - 1; x++) {
			for (int z = 1; z < snapshots[0][0].length - 1; z++) {
				for (int y = miny; y < snapshots[0].length - 1; y++) {
					...
					if (underBlock.typeId == Material.AIR.getId()
							|| underBlock.typeId == Material.STATIONARY_WATER
									.getId()
							|| underBlock.typeId == Material.STATIONARY_LAVA
									.getId()
	...
	private void removeWallsAndTowers() {
		int[] excludedBlocksArray = new int[] { Material.CACTUS.getId(),
				Material.LONG_GRASS.getId(), Material.RED_MUSHROOM.getId(),
				Material.BROWN_MUSHROOM.getId(), Material.DEAD_BUSH.getId(),
				Material.SAPLING.getId(), Material.YELLOW_FLOWER.getId(),
				Material.RED_ROSE.getId(), Material.SUGAR_CANE_BLOCK.getId(),
				Material.VINE.getId(), Material.PUMPKIN.getId(),
				Material.WATER_LILY.getId(), Material.LEAVES.getId() };
		...
		do {
			...
			for (int x = 1; x < snapshots.length - 1; x++) {
				for (int z = 1; z < snapshots[0][0].length - 1; z++) {
					...
					if (excludedBlocks
							.contains(this.snapshots[x][thisy][z].typeId))
						continue;
				...
				for (int z = 1; z < snapshots[0][0].length - 1; z++) {
					for (int y = 0; y < snapshots[0].length - 1; y++) {
						...
						if (!fillableBlocks.contains(leftBlock.typeId)
							...
							if (!notSuitableForFillBlocks
									.contains(rightBlock.typeId)) {
				...
				for (int x = 1; x < snapshots.length - 1; x++) {
					for (int z = 1; z < snapshots[0][0].length - 1; z++) {
						...
						if (block.typeId == Material.AIR.getId()
							...
							if (underBlock.typeId != Material.STATIONARY_WATER
									.getId() || underBlock.data != 0)
								continue;

							// count adjacent source water blocks
							byte adjacentSourceWaterCount = 0;
							if (leftBlock.typeId == Material.STATIONARY_WATER
									.getId() && leftBlock.data == 0) {
								adjacentSourceWaterCount++;
							}
							if (rightBlock.typeId == Material.STATIONARY_WATER
									.getId() && rightBlock.data == 0) {
								adjacentSourceWaterCount++;
							}
							if (upBlock.typeId == Material.STATIONARY_WATER
									.getId() && upBlock.data == 0) {
								adjacentSourceWaterCount++;
							}
							if (downBlock.typeId == Material.STATIONARY_WATER
									.getId() && downBlock.data == 0) {
								adjacentSourceWaterCount++;
							}
	...
	public static ArrayList<Integer> getPlayerBlocks(Environment environment,
			Biome biome) {
		...
		if (biome == Biome.DESERT || biome == Biome.DESERT_HILLS
				|| biome == Biome.BEACH || environment != Environment.NORMAL) {
