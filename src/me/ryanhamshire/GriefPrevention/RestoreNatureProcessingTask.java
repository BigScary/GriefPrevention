...
class RestoreNatureProcessingTask implements Runnable 
{
    ...
    private void coverSurfaceStone()
	{
		for(int x = 1; x < snapshots.length - 1; x++)
		{
			for(int z = 1; z < snapshots[0][0].length - 1; z++)
			{
				...
				{
					if(this.biome == Biome.DESERT || this.biome == Biome.DESERT_HILLS || this.biome == Biome.COLD_BEACH)
					{
    ...
    static ArrayList<Integer> getPlayerBlocks(Environment environment, Biome biome) 
	{
		...
		if(biome == Biome.DESERT || biome == Biome.DESERT_HILLS || biome == Biome.COLD_BEACH || environment != Environment.NORMAL)
		{
