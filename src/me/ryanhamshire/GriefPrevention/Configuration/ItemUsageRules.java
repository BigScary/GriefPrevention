...
public class ItemUsageRules {
	...
	public ClaimAllowanceConstants TestPlayer(Player p, Block Target) {
		ClaimAllowanceConstants PlayerLoc = PlayerLocationRules.checkPlacement(
				p.getLocation(), p);
		ClaimAllowanceConstants TargetLoc = TargetLocationRules.checkPlacement(
				Target.getLocation(), p);
		if (PlayerLoc.Denied()) {
			GriefPrevention.sendMessage(p, TextMode.Err,
					"You may not use that Item Here.");
			return ClaimAllowanceConstants.Deny;
		} else if (TargetLoc.Denied()) {
			GriefPrevention.sendMessage(p, TextMode.Err,
					"You may not use that Item There.");
			return ClaimAllowanceConstants.Deny;
		}
	...
	public List<ItemUsageRules> ReadRules(String BaseNode,
			FileConfiguration Source, FileConfiguration Target) {
		ArrayList<ItemUsageRules> BuildList = new ArrayList<ItemUsageRules>();
		ConfigurationSection grabsection = Source
				.getConfigurationSection(BaseNode);

		for (String iterate : grabsection.getKeys(false)) {
			ItemUsageRules newelement = new ItemUsageRules(BaseNode + "."
	...
	public ItemUsageRules(String NodePath, FileConfiguration Source,
		...
		PlayerLocationRules = new BlockPlacementRules(
				"Item Use at Player Location", Source, Target, NodePath
						+ ".PlayerLocation");
		TargetLocationRules = new BlockPlacementRules(
				"Item Use Target Location", Source, Target, NodePath
						+ ".TargetLocation");
