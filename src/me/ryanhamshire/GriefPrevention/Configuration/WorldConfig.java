...
public class WorldConfig {
	...
	// private boolean config_claims_AllowEnvironmentalVehicleDamage; //whether
	// Entities can take damage from the environment in a claim.
	// public boolean claims_AllowEnvironmentalVehicleDamage(){return
	// config_claims_AllowEnvironmentalVehicleDamage;}
	...
	public int getafkDistanceSquared() {
		...
		return afkDistanceSquared != -1 ? afkDistanceSquared
				: (afkDistanceSquared = (config_afkDistanceCheck * config_afkDistanceCheck));
	}
	...
	public Integer getSeaLevelOverride() {
		if (config_seaLevelOverride == -1)
			return (config_seaLevelOverride = Bukkit.getWorld(
					this.getWorldName()).getSeaLevel() - 1);
		else
	...
	public WorldConfig(String pName, FileConfiguration config,
			FileConfiguration outConfig) {
		...
		if (getworld != null) {
			...
			boolean gotpvp = config.getBoolean("GriefPrevention.PvP.Enabled",
					isPvP);
			outConfig.set("GriefPrevention.PvP.Enabled", gotpvp);
			getworld.setPVP(isPvP = gotpvp);
		}

		Debugger.Write("Reading Configuration for World:" + pName,
				DebugLevel.Verbose);

		this.config_seaLevelOverride = config.getInt(
				"GriefPrevention.SeaLevelOverride", -1);

		if (this.config_seaLevelOverride == -1) {

			// try to get new setting.
			int newsealevel = config.getInt(
					"GriefPrevention.SeaLevelOverride.Setting", -1);
			boolean dooverride = config.getBoolean(
					"GriefPrevention.SeaLevelOverride.Enabled", false);
		...
		if (config_seaLevelOverride != -1) {
			outConfig.set("GriefPrevention.SeaLevelOverride.Setting",
					config_seaLevelOverride);
		}
		outConfig.set("GriefPrevention.SeaLevelOverride.Enabled",
				config_seaLevelOverride != -1);

		// read in the data for TNT explosions and Golem/Wither placements.
		this.config_afkDistanceCheck = config.getInt(
				"GriefPrevention.AFKDistance", 3);
		this.SilverfishBreakRules = new ClaimBehaviourData("Silverfish Break",
				config, outConfig, "GriefPrevention.Rules.SilverfishBreak",
				new ClaimBehaviourData("Silverfish Break", PlacementRules.Both,
						PlacementRules.Neither, ClaimBehaviourMode.Disabled));

		this.CreeperExplosionsBehaviour = new ClaimBehaviourData(
				"Creeper Explosions", config, outConfig,
				"GriefPrevention.Rules.CreeperExplosions",
				new ClaimBehaviourData("Creeper Explosions",
						PlacementRules.Both, PlacementRules.Both,
						ClaimBehaviourMode.Disabled));
		this.WitherExplosionBehaviour = new ClaimBehaviourData(
				"Wither Explosions", config, outConfig,
				"GriefPrevention.Rules.WitherExplosions",
				new ClaimBehaviourData("Wither Explosions",
						PlacementRules.Neither, PlacementRules.Neither,
						ClaimBehaviourMode.Disabled));

		this.TNTExplosionsBehaviour = new ClaimBehaviourData("TNT Explosions",
				config, outConfig, "GriefPrevention.Rules.TNTExplosions",

				ClaimBehaviourData.getAll("TNT Explosions"));

		this.OtherExplosionBehaviour = new ClaimBehaviourData(
				"Other Explosions", config, outConfig,
				"GriefPrevention.Rules.OtherExplosions",
				ClaimBehaviourData.getAll("Other Explosions"));

		this.CreeperExplosionBlockDamageBehaviour = new ClaimBehaviourData(
				"Creeper Explosion Damage", config, outConfig,
				"GriefPrevention.Rules.BlockDamageCreeperExplosion",
				new ClaimBehaviourData("Creeper Explosion Damage",
						PlacementRules.Both, PlacementRules.Neither,
						ClaimBehaviourMode.Disabled));

		this.WitherExplosionBlockDamageBehaviour = new ClaimBehaviourData(
				"Wither Explosion Damage", config, outConfig,
				"GriefPrevention.Rules.BlockDamageWitherExplosions",
				new ClaimBehaviourData("Wither Explosion Damage",
						PlacementRules.Both, PlacementRules.Neither,
						ClaimBehaviourMode.Disabled));

		this.WitherEatBehaviour = new ClaimBehaviourData("Wither Eating",
				config, outConfig, "GriefPrevention.Rules.WitherEating",
				new ClaimBehaviourData("Wither Eating", PlacementRules.Both,
						PlacementRules.Neither, ClaimBehaviourMode.Disabled));

		this.TNTExplosionBlockDamageBehaviour = new ClaimBehaviourData(
				"TNT Explosion Damage", config, outConfig,
				"GriefPrevention.Rules.BlockDamageTNTExplosions",
				ClaimBehaviourData.getOutsideClaims("TNT Explosion Damage"));

		this.OtherExplosionBlockDamageBehaviour = new ClaimBehaviourData(
				"Other Explosion Damage", config, outConfig,
				"GriefPrevention.Rules.BlockDamageOtherExplosions",
				ClaimBehaviourData.getOutsideClaims("Other Explosion Damage"));

		ClaimBehaviourData WaterRequire = new ClaimBehaviourData(
				"Water Placement", PlacementRules.BelowOnly,
				PlacementRules.Both, ClaimBehaviourMode.RequireBuild);
		ClaimBehaviourData LavaRequire = new ClaimBehaviourData(
				"Lava Placement", PlacementRules.BelowOnly,
				PlacementRules.Both, ClaimBehaviourMode.RequireBuild)
				.setWildernessRequiredPermission("GriefPrevention.Lava");

		this.WaterBucketEmptyBehaviour = new ClaimBehaviourData(
				"Water Placement", config, outConfig,
				"GriefPrevention.Rules.WaterBuckets.Place", WaterRequire);

		this.LavaBucketEmptyBehaviour = new ClaimBehaviourData(
				"Lava Placement", config, outConfig,
				"GriefPrevention.Rules.LavaBuckets.Place", LavaRequire);
		this.WaterBucketFillBehaviour = new ClaimBehaviourData(
				"Water Bucket Fill", config, outConfig,
				"GriefPrevention.Rules.WaterBuckets.Fill", ClaimBehaviourData
						.getAll("Water Bucket Fill").setBehaviourMode(
								ClaimBehaviourMode.RequireBuild));

		this.LavaBucketFillBehaviour = new ClaimBehaviourData(
				"Lava Bucket Fill", config, outConfig,
				"GriefPrevention.Rules.LavaBuckets.Fill", ClaimBehaviourData
						.getAll("Lava Bucket Fill").setBehaviourMode(
								ClaimBehaviourMode.RequireBuild));
		// Snow golem spawn rules.

		this.IronGolemSpawnBehaviour = new ClaimBehaviourData(
				"Iron Golem Spawning", config, outConfig,
				"GriefPrevention.Rules.BuildIronGolem",
				ClaimBehaviourData.getInsideClaims("Iron Golem Spawning"));

		this.SnowGolemSpawnBehaviour = new ClaimBehaviourData(
				"Snow Golem Spawning", config, outConfig,
				"GriefPrevention.Rules.BuildSnowGolem",
				ClaimBehaviourData.getInsideClaims("Snow Golem Spawning"));

		this.WitherSpawnBehaviour = new ClaimBehaviourData("Wither Spawning",
				config, outConfig, "GriefPrevention.Rules.BuildWither",
				ClaimBehaviourData.getInsideClaims("Wither Spawning"));

		TrashBlockPlacementBehaviour = new ClaimBehaviourData(
				"Trash Block Placement", config, outConfig,
				"GriefPrevention.Rules.TrashBlockPlacementRules",
				ClaimBehaviourData.getOutsideClaims("Trash Block Placement")
						.setBehaviourMode(ClaimBehaviourMode.RequireBuild));

		VillagerTrades = new ClaimBehaviourData("Villager Trading", config,
				outConfig, "GriefPrevention.Rules.VillagerTrading",
				ClaimBehaviourData.getInsideClaims("Villager Trading")
						.setBehaviourMode(ClaimBehaviourMode.RequireContainer));

		this.EnvironmentalVehicleDamage = new ClaimBehaviourData(
				"Environmental Vehicle Damage", config, outConfig,
				"GriefPrevention.Rules.EnvironmentalVehicleDamage",
				ClaimBehaviourData
						.getOutsideClaims("Environmental Vehicle Damage"));

		this.ZombieDoorBreaking = new ClaimBehaviourData(
				"Zombie Door Breaking", config, outConfig,
				"GriefPrevention.Rules.ZombieDoorBreaking",
				ClaimBehaviourData.getNone("Zombie Door Breaking"));

		SheepShearingRules = new ClaimBehaviourData("Sheep Shearing", config,
				outConfig, "GriefPrevention.Rules.SheepShearing",
				ClaimBehaviourData.getAll("Sheep Shearing").setBehaviourMode(
						ClaimBehaviourMode.RequireContainer));

		SheepDyeing = new ClaimBehaviourData("Sheep Dyeing", config, outConfig,
				"GriefPrevention.Rules.SheepDyeing", ClaimBehaviourData.getAll(
						"Sheep Dyeing").setBehaviourMode(
						ClaimBehaviourMode.RequireContainer));

		this.BonemealGrass = new ClaimBehaviourData("Bonemeal", config,
				outConfig, "GriefPrevention.Rules.BonemealGrass",
				ClaimBehaviourData.getAll("Bonemeal").setBehaviourMode(
						ClaimBehaviourMode.RequireBuild));

		this.PlayerTrampleRules = new ClaimBehaviourData("Crop Trampling",
				config, outConfig, "GriefPrevention.Rules.PlayerCropTrample",
				ClaimBehaviourData.getAll("Crop Trampling").setBehaviourMode(
						ClaimBehaviourMode.RequireBuild));

		this.EndermanPickupRules = new ClaimBehaviourData("Enderman Pickup",
				config, outConfig, "GriefPrevention.Rules.EndermanPickup",
				ClaimBehaviourData.getNone("Enderman Pickup"));

		this.EndermanPlacementRules = new ClaimBehaviourData(
				"Enderman Placement", config, outConfig,
				"GriefPrevention.Rules.EndermanPlacement",
				ClaimBehaviourData.getNone("Enderman Placement"));

		this.ArrowWoodenButtonRules = new ClaimBehaviourData(
				"Arrows Trigger Wood Buttons", config, outConfig,
				"GriefPrevention.Rules.ArrowsHitWoodButtons",
				ClaimBehaviourData.getAll("Arrows Trigger Wood Buttons")
						.setBehaviourMode(ClaimBehaviourMode.RequireAccess));

		this.BlockTweakRules = new ClaimBehaviourData("Block Tweaking", config,
				outConfig, "GriefPrevention.Rules.BlockTweaking",
				ClaimBehaviourData.getAll("Block Tweaking").setBehaviourMode(
						ClaimBehaviourMode.RequireAccess));

		this.ContainersRules = new ClaimBehaviourData("Containers", config,
				outConfig, "GriefPrevention.Rules.Containers",
				ClaimBehaviourData.getAll("Containers").setBehaviourMode(
						ClaimBehaviourMode.RequireContainer)); // defaults to
																// only allowing
																// theft outside
																// claims.

		this.CreatureDamage = new ClaimBehaviourData("Creature Damage", config,
				outConfig, "GriefPrevention.Rules.CreatureDamage",
				ClaimBehaviourData.getAll("Creature Damage").setBehaviourMode(
						ClaimBehaviourMode.RequireContainer));

		this.WoodenDoors = new ClaimBehaviourData("Wooden Doors", config,
				outConfig, "GriefPrevention.Rules.WoodenDoors",
				ClaimBehaviourData.getAll("Wooden Doors").setBehaviourMode(
						ClaimBehaviourMode.RequireNone));

		this.TrapDoors = new ClaimBehaviourData("TrapDoors", config, outConfig,
				"GriefPrevention.Rules.TrapDoors", ClaimBehaviourData.getAll(
						"TrapDoors").setBehaviourMode(
						ClaimBehaviourMode.RequireNone));

		this.FenceGates = new ClaimBehaviourData("Fence Gates", config,
				outConfig, "GriefPrevention.Rules.FenceGates",
				ClaimBehaviourData.getAll("Fence Gates").setBehaviourMode(
						ClaimBehaviourMode.RequireNone));

		this.EnderPearlOrigins = new ClaimBehaviourData("EnderPearl Origins",
				config, outConfig, "GriefPrevention.Rules.EnderPearlOrigin",
				ClaimBehaviourData.getAll("EnderPearl Origins")
						.setBehaviourMode(ClaimBehaviourMode.RequireAccess));

		this.EnderPearlTargets = new ClaimBehaviourData("EnderPearl Targets",
				config, outConfig, "GriefPrevention.Rules.EnderPearlTarget",
				ClaimBehaviourData.getAll("EnderPearl Targets")
						.setBehaviourMode(ClaimBehaviourMode.RequireAccess));

		this.StonePressurePlates = new ClaimBehaviourData(
				"Stone Pressure Plates", config, outConfig,
				"GriefPrevention.Rules.StonePressurePlates", ClaimBehaviourData
						.getAll("Stone Pressure Plates").setBehaviourMode(
								ClaimBehaviourMode.RequireNone));

		this.WoodPressurePlates = new ClaimBehaviourData(
				"Wooden Pressure Plates", config, outConfig,
				"GriefPrevention.Rules.WoodenPressurePlates",
				ClaimBehaviourData.getAll("Wooden Pressure Plates")
						.setBehaviourMode(ClaimBehaviourMode.RequireNone));
		this.ArrowWoodenTouchplateRules = new ClaimBehaviourData(
				"Wooden Touchplate", config, outConfig,
				"GriefPrevention.Rules.ArrowWoodenPressurePlate",
				ClaimBehaviourData.getAll("Wooden Pressure Plates Arrows")
						.setBehaviourMode(ClaimBehaviourMode.RequireNone));
		this.StoneButton = new ClaimBehaviourData("Stone Button", config,
				outConfig, "GriefPrevention.Rules.StoneButton",
				ClaimBehaviourData.getAll("Stone Button").setBehaviourMode(
						ClaimBehaviourMode.RequireAccess));

		this.WoodenButton = new ClaimBehaviourData("Wooden Button", config,
				outConfig, "GriefPrevention.Rules.WoodenButton",
				ClaimBehaviourData.getAll("Wooden Button").setBehaviourMode(
						ClaimBehaviourMode.RequireAccess));

		this.Levers = new ClaimBehaviourData("Levers", config, outConfig,
				"GriefPrevention.Rules.Levers", ClaimBehaviourData.getAll(
						"Levers").setBehaviourMode(
						ClaimBehaviourMode.RequireAccess));

		this.Beds = new ClaimBehaviourData("Beds", config, outConfig,
				"GriefPrevention.Rules.Beds", ClaimBehaviourData.getAll("Beds")
						.setBehaviourMode(ClaimBehaviourMode.RequireAccess));

		this.FireExtinguishing = new ClaimBehaviourData("Fire Extinguishing",
				config, outConfig, "GriefPrevention.Rules.FireExtinguishing",
				ClaimBehaviourData.getAll("Fire Extinguishing")
						.setBehaviourMode(ClaimBehaviourMode.RequireBuild));

		this.FireSetting = new ClaimBehaviourData(
				"Fire Setting",
				config,
				outConfig,
				"GriefPrevention.Rules.FireSetting",
				ClaimBehaviourData
						.getInsideClaims("Fire Setting")
						.setBehaviourMode(ClaimBehaviourMode.RequireBuild)
						.setWildernessRequiredPermission("GriefPrevention.Lava"));
		...
		// default.
		this.BreedingRules = new ClaimBehaviourData("Animal Breeding", config,
				outConfig, "GriefPrevention.Rules.Breeding", ClaimBehaviourData
						.getInsideClaims("Animal Breeding").setBehaviourMode(
								ClaimBehaviourMode.RequireAccess));
		...
		this.TamingRules = new ClaimBehaviourData("Taming Rules", config,
				outConfig, "GriefPrevention.Rules.Taming", ClaimBehaviourData
						.getAll("Taming Rules").setBehaviourMode(
								ClaimBehaviourMode.RequireAccess));
		...
		// claim.
		this.LeadUsageRules = new ClaimBehaviourData("Lead Usage", config,
				outConfig, "GriefPrevention.Rules.LeadUsage",
				ClaimBehaviourData.getAll("Lead Usage").setBehaviourMode(
						ClaimBehaviourMode.RequireAccess));
		...
		this.EquineInventoryRules = new ClaimBehaviourData("Equine Inventory",
				config, outConfig, "GriefPrevention.Rules.EquineInventory",
				ClaimBehaviourData.getNone("Equine Inventory"));

		// Name Tag usage.
		// Will work in the wild. Requires Container trust inside of claims.
		this.NameTagUsageRules = new ClaimBehaviourData("Name Tags", config,
				outConfig, "GriefPrevention.Rules.NameTags", ClaimBehaviourData
						.getAll("Nem Tags").setBehaviourMode(
								ClaimBehaviourMode.RequireContainer));
		...
		this.FeedingRules = new ClaimBehaviourData("Feeding", config,
				outConfig, "GriefPrevention.Rules.Feeding", ClaimBehaviourData
						.getAll("Feeding").setBehaviourMode(
								ClaimBehaviourMode.RequireAccess));

		// Dispenser rules.
		this.DispenserLavaBehaviour = new ClaimBehaviourData("Lava Dispensing",
				config, outConfig, "GriefPrevention.Rules.Dispensers.Lava",
				ClaimBehaviourData.getInsideClaims("Lava Dispensing"));

		this.DispenserWaterBehaviour = new ClaimBehaviourData(
				"Water Dispensing", config, outConfig,
				"GriefPrevention.Rules.Dispensers.Water",
				ClaimBehaviourData.getAll("Water Dispensing"));

		this.DispenserArrowBehaviour = new ClaimBehaviourData(
				"Arrow Dispensing", config, outConfig,
				"GriefPrevention.Rules.Dispensers.Arrows",
				ClaimBehaviourData.getInsideClaims("Arrow Dispensing"));

		this.DispenserFireChargeBehaviour = new ClaimBehaviourData(
				"Fire Charge Dispensing", config, outConfig,
				"GriefPrevention.Rules.Dispensers.FireCharges",
				ClaimBehaviourData.getInsideClaims("Fire Charge Dispensing"));

		this.DispenserFlintandSteelBehaviour = new ClaimBehaviourData(
				"Flint and Steel Dispensing", config, outConfig,
				"GriefPrevention.Rules.Dispensers.FlintandSteel",
				ClaimBehaviourData
						.getInsideClaims("Flint and Steel Dispensing"));

		this.DispenserPositivePotionBehaviour = new ClaimBehaviourData(
				"Positive Potion Dispensing", config, outConfig,
				"GriefPrevention.Rules.Dispensers.PositivePotions",
				ClaimBehaviourData.getAll("Positive Potion Dispensing"));

		this.DispenserNegativePotionBehaviour = new ClaimBehaviourData(
				"Negative Potion Dispensing", config, outConfig,
				"GriefPrevention.Rules.Dispensers.NegativePotions",
				ClaimBehaviourData.getAll("Negative Potion Dispensing"));

		this.DispenserSpawnEggBehaviour = new ClaimBehaviourData(
				"Spawn Egg Dispensing", config, outConfig,
				"GriefPrevention.Rules.Dispensers.SpawnEggs",
				ClaimBehaviourData.getAll("Spawn Egg Dispensing"));

		this.DispenserFireworkBehaviour = new ClaimBehaviourData(
				"Firework Dispensing", config, outConfig,
				"GriefPrevention.Rules.Dispensers.Fireworks",
				ClaimBehaviourData.getAll("Firework Dispensing"));

		this.DispenserSnowballBehaviour = new ClaimBehaviourData(
				"Snowball Dispensing", config, outConfig,
				"GriefPrevention.Rules.Dispensers.Snowballs",
				ClaimBehaviourData.getAll("Snowball Dispensing"));
		this.DispenserEggBehaviour = new ClaimBehaviourData("Egg Dispensing",
				config, outConfig, "GriefPrevention.Rules.Dispensers.Eggs",
				ClaimBehaviourData.getInsideClaims("Egg Dispensing"));
		this.DispenserMiscBehaviour = new ClaimBehaviourData("Misc Dispensing",
				config, outConfig, "GriefPrevention.Rules.Dispensers.Misc",
		...
		this.SiegeAutoTransfer = config.getBoolean(
				"GriefPrevention.Siege.ItemTransfer", true);
		outConfig.set("GriefPrevention.Siege.ItemTransfer",
				this.SiegeAutoTransfer);

		this.SiegeBlockRevert = config.getBoolean(
				"GriefPrevention.Siege.BlockRevert", false);
		...
		// SpamBanThreshold=8
		// SpamMuteThreshold=4
		this.config_SpamDelayThreshold = config.getInt(
				"GriefPrevention.Spam.DelayThreshold", 1500);
		outConfig.set("GriefPrevention.Spam.DelayThreshold",
				config_SpamDelayThreshold);
		this.config_SpamCapsMinLength = config.getInt(
				"GriefPrevention.Spam.CapsMinLength", 4);
		outConfig.set("GriefPrevention.Spam.CapsMinLength",
				config_SpamCapsMinLength);
		this.config_SpamNonAlphaNumMinLength = config.getInt(
				"GriefPrevention.Spam.NonAlphaNumMinLength", 5);
		outConfig.set("GriefPrevention.Spam.AlphaNumMinLength",
				config_SpamNonAlphaNumMinLength);
		this.config_SpamASCIIArtMinLength = config.getInt(
				"GriefPrevention.Spam.ASCIIArtMinLength", 15);
		outConfig.set("GriefPrevention.Spam.ASCIIArtMinLength",
				config_SpamASCIIArtMinLength);
		this.config_SpamShortMessageMaxLength = config.getInt(
				"GriefPrevention.Spam.ShortMessageMaxLength", 5);
		outConfig.set("GriefPrevention.Spam.ShortMessageMaxLength",
				config_SpamShortMessageMaxLength);
		this.config_SpamKickThreshold = config.getInt(
				"GriefPrevention.Spam.KickThreshold", 6);
		outConfig.set("GriefPrevention.Spam.KickThreshold",
				config_SpamKickThreshold);
		this.config_SpamBanThreshold = config.getInt(
				"GriefPrevention.Spam.BanThreshold", 8);
		outConfig.set("GriefPrevention.Spam.BanThreshold",
				config_SpamBanThreshold);
		this.config_SpamMuteThreshold = config.getInt(
				"GriefPrevention.Spam.MuteThreshold", 4);
		outConfig.set("GriefPrevention.Spam.MuteThreshold",
				config_SpamMuteThreshold);

		this.InsufficientSneakResetBound = config.getInt(
				"GriefPrevention.Claims.InsufficientSneakResetBound", 0);
		outConfig.set("GriefPrevention.Claims.InsufficientSneakResetBound",
				this.InsufficientSneakResetBound);
		this.claims_Seige_Enabled = config.getBoolean(
				"GriefPrevention.Siege.Enabled", isPvP);
		outConfig.set("GriefPrevention.Siege.Enabled", claims_Seige_Enabled);

		this.Siege_TamedAnimalDistance = config.getInt(
				"GriefPrevention.Claims.SiegeTamedAnimalDistance", 20);
		outConfig.set("GriefPrevention.Claims.SiegeTamedAnimalDistance",
				Siege_TamedAnimalDistance);
		this.claims_enabled = config.getBoolean(
				"GriefPrevention.Claims.Enabled", true);
		outConfig.set("GriefPrevention.Claims.Enabled", claims_enabled);
		this.config_entitycleanup_enabled = config.getBoolean(
				"GriefPrevention.CleanupTasks.Claims", true);
		outConfig.set("GriefPrevention.CleanupTasks.Entity",
				this.config_entitycleanup_enabled);
		...
		this.config_claimcleanup_enabled = config.getBoolean(
				"GriefPrevention.ClaimCleanup.Enabled", true);
		this.config_claimcleanup_maximumsize = config.getInt(
				"GriefPrevention.ClaimCleanup.MaximumSize", 25);
		// max investment score, defaults to 400 for creative worlds.
		this.config_claimcleanup_maxinvestmentscore = config.getInt(
				"GriefPrevention.ClaimCleanup.MaxInvestmentScore",
				isCreative ? 400 : 100);

		config_claims_blocksAccruedPerHour = config.getInt(
				"GriefPrevention.Claims.BlocksAccruedPerHour", 100);

		this.config_claims_maxblocks = config.getInt(
				"GriefPrevention.Claims.MaxClaimBlocks", 0);
		outConfig.set("GriefPrevention.Claims.MaxClaimBlocks",
				this.config_claims_maxblocks);

		outConfig.set("GriefPrevention.Claims.BlocksAccruedPerHour",
				config_claims_blocksAccruedPerHour);
		outConfig.set("GriefPrevention.ClaimCleanup.MaximumSize",
				config_claimcleanup_maximumsize);
		outConfig.set("GriefPrevention.ClaimCleanup.MaxInvestmentScore",
				this.config_claimcleanup_maxinvestmentscore);
		outConfig.set("GriefPrevention.ClaimCleanup.Enabled",
				this.config_claimcleanup_enabled);
		this.config_message_cooldown_claims = config.getInt(
				"GriefPrevention.Expiration.MessageCooldown.Claim", 0);
		this.config_message_cooldown_stuck = config.getInt(
				"GriefPrevention.Expiration.MessageCooldown.Stuck", 0);
		outConfig.set("GriefPrevention.Expiration.MessageCooldown.Claim",
				config_message_cooldown_claims);
		outConfig.set("GriefPrevention.Expiration.MessageCooldown.Stuck",
				config_message_cooldown_stuck);

		this.config_claims_wildernessBlocksDelay = config.getInt(
				"GriefPrevention.Claims.WildernessWarningBlockCount", 15); // number
																			// of
																			// blocks,0
																			// will
																			// disable
																			...
																			// wilderness
																			// warning.
		this.config_claims_creativeRules = config.getBoolean(
				"GriefPrevention.CreativeRules", Bukkit.getServer()
						.getDefaultGameMode() == GameMode.CREATIVE);
		this.config_claims_AbandonReturnRatio = config.getDouble(
				"GriefPrevention.Claims.AbandonReturnRatio", 1);
		outConfig.set("GriefPrevention.Claims.AbandonReturnRatio",
				this.config_claims_AbandonReturnRatio);
		outConfig.set("GriefPrevention.CreativeRules",
				config_claims_creativeRules);

		this.config_claims_showsurroundingsRadius = config.getInt(
				"GriefPrevention.Claims.InvestigateSurroundingsRadius", 50);

		this.config_claims_perplayer_claim_limit = config.getInt(
				"GriefPrevention.Claims.PerPlayerLimit", 0);

		outConfig.set("GriefPrevention.Claims.PerPlayerLimit",
				config_claims_perplayer_claim_limit);

		this.config_pvp_Seige_Loot_Chests = config.getInt(
				"GriefPrevention.Claims.SeigeLootChests", 0);
		outConfig.set("GriefPrevention.Claims.SeigeLootChests",
				config_pvp_Seige_Loot_Chests);

		this.config_claims_automaticClaimsForNewPlayersRadius = config.getInt(
				"GriefPrevention.Claims.AutomaticNewPlayerClaimsRadius", 4);
		this.config_claims_claimsExtendIntoGroundDistance = config.getInt(
				"GriefPrevention.Claims.ExtendIntoGroundDistance", 5);
		this.config_claims_creationRequiresPermission = config.getBoolean(
				"GriefPrevention.Claims.CreationRequiresPermission", false);
		this.config_claims_minBlocks = config.getInt(
				"GriefPrevention.Claims.MinimumBlocks", 100);
		this.config_claims_minSize = config.getInt(
				"GriefPrevention.Claims.MinimumSize", 10);
		this.config_claims_maxDepth = config.getInt(
				"GriefPrevention.Claims.MaximumDepth", 0);
		this.config_claims_trappedCooldownMinutes = config.getInt(
				"GriefPrevention.Claims.TrappedCommandCooldownMinutes", 8 * 60);

		this.config_claims_ApplyTrashBlockRules = config
				.getBoolean(
						"GriefPrevention.Claims.NoSurvivalBuildingOutsideClaims",
						false);
		this.config_claims_warnOnBuildOutside = config.getBoolean(
				"GriefPrevention.Claims.WarnWhenBuildingOutsideClaims", true);
		this.config_claims_allowUnclaim = config.getBoolean(
				"GriefPrevention.Claims.AllowUnclaimingLand", true);
		this.config_claims_autoRestoreUnclaimed = config.getBoolean(
				"GriefPrevention.Claims.AutoRestoreUnclaimedLand", true);

		this.config_claims_Abandon_NatureRestoration = config.getBoolean(
				"GriefPrevention.Claims.AbandonAutoRestore", false);
		outConfig.set("GriefPrevention.Claims.AbandonAutoRestore",
				this.config_claims_Abandon_NatureRestoration);
		...
		this.config_claims_chestClaimExpirationDays = config.getInt(
				"GriefPrevention.Claims.Expiration.ChestClaimDays", 7);
		outConfig.set("GriefPrevention.Claims.Expiration.ChestClaimDays",
				this.config_claims_chestClaimExpirationDays);

		this.config_claims_unusedClaimExpirationDays = config.getInt(
				"GriefPrevention.Claims.Expiration.UnusedClaimDays", 14);
		...
		this.config_claims_expirationDays = config.getInt(
				"GriefPrevention.Claims.Expiration.AllClaimDays", 0);
		outConfig.set("GriefPrevention.Claims.Expiration.AllClaimDays",
				this.config_claims_expirationDays);

		this.config_claims_AutoNatureRestoration = config.getBoolean(
				"GriefPrevention.Claims.Expiration.AutomaticNatureRestoration",
				false);
		outConfig.set(
				"GriefPrevention.Claims.Expiration.AutomaticNatureRestoration",
				this.config_claims_AutoNatureRestoration);

		this.config_spam_enabled = config.getBoolean(
				"GriefPrevention.Spam.Enabled", true);
		int cooldownminutes = config.getInt(
				"GriefPrevention.Spam.LoginCooldownMinutes", -1);
		if (cooldownminutes == -1) {
			this.config_spam_loginCooldownSeconds = config.getInt(
					"GriefPrevention.Spam.LoginCooldownSeconds", 60);
		} else
			this.config_spam_loginCooldownSeconds = cooldownminutes * 60;

		outConfig.set("GriefPrevention.Spam.LoginCooldownSeconds",
				config_spam_loginCooldownSeconds);
		...
		this.config_spam_allowedIpAddresses = config.getString(
				"GriefPrevention.Spam.AllowedIpAddresses", "1.2.3.4; 5.6.7.8");
		this.config_spam_banMessage = config.getString(
				"GriefPrevention.Spam.BanMessage", "Banned for spamming.");
		this.config_spam_kickMessage = config.getString(
				"GriefPrevention.Spam.KickMessage", "Kicked for spamming.");
		String slashCommandsToMonitor = config.getString(
				"GriefPrevention.Spam.MonitorSlashCommands",
				"/me;/tell;/global;/local");
		this.config_spam_monitorSlashCommands = Arrays
				.asList(slashCommandsToMonitor.split(";"));
		this.config_spam_deathMessageCooldownSeconds = config.getInt(
				"GriefPrevention.Spam.DeathMessageCooldownSeconds", 60);

		this.config_spam_kickcommand = config.getString(
				"GriefPrevention.Spam.KickCommand", "kick {0}");
		this.config_spam_bancommand = config.getString(
				"GriefPrevention.Spam.BanCommand", "ban {0};kick {0}");

		outConfig.set("GriefPrevention.Spam.Enabled", this.config_spam_enabled);

		//
		outConfig.set("GriefPrevention.Spam.MonitorSlashCommands",
				slashCommandsToMonitor);
		outConfig.set("GriefPrevention.Spam.WarningMessage",
				this.config_spam_warningMessage);
		outConfig.set("GriefPrevention.Spam.KickMessage",
				this.config_spam_kickMessage);
		outConfig.set("GriefPrevention.Spam.BanMessage",
				this.config_spam_banMessage);
		outConfig.set("GriefPrevention.Spam.AllowedIpAddresses",
				this.config_spam_allowedIpAddresses);
		outConfig.set("GriefPrevention.Spam.DeathMessageCooldownSeconds",
				this.config_spam_deathMessageCooldownSeconds);

		outConfig.set("GriefPrevention.Spam.KickCommand",
				this.config_spam_kickcommand);
		outConfig
				.set("GriefPrevention.Spam.BanCommand", config_spam_bancommand);

		this.config_pvp_protectFreshSpawns = config.getBoolean(
				"GriefPrevention.PvP.ProtectFreshSpawns", true);
		this.config_pvp_punishLogout = config.getBoolean(
				"GriefPrevention.PvP.PunishLogout", true);
		this.config_pvp_combatTimeoutSeconds = config.getInt(
				"GriefPrevention.PvP.CombatTimeoutSeconds", 15);
		this.config_pvp_allowCombatItemDrop = config.getBoolean(
				"GriefPrevention.PvP.AllowCombatItemDrop", false);
		this.config_pvp_blockContainers = config.getBoolean(
				"GriefPrevention.PvP.BlockContainers", true);
		String bannedPvPCommandsList = config.getString(
				"GriefPrevention.PvP.BlockedSlashCommands",
				"/home;/vanish;/spawn;/tpa");

		this.config_trees_removeFloatingTreetops = config.getBoolean(
				"GriefPrevention.Trees.RemoveFloatingTreetops", true);
		this.config_trees_regrowGriefedTrees = config.getBoolean(
				"GriefPrevention.Trees.RegrowGriefedTrees", true);
		outConfig.set("GriefPrevention.PvP.BlockContainers",
				config_pvp_blockContainers);

		// this.config_blockWildernessWaterBuckets =
		// config.getBoolean("GriefPrevention.LimitSurfaceWaterBuckets", true);
		this.config_blockSkyTrees = config.getBoolean(
				"GriefPrevention.LimitSkyTrees", true);

		this.config_fireSpreads = config.getBoolean(
				"GriefPrevention.FireSpreads", false);
		this.config_fireDestroys = config.getBoolean(
				"GriefPrevention.FireDestroys", false);

		this.config_addItemsToClaimedChests = config.getBoolean(
				"GriefPrevention.AddItemsToClaimedChests", true);
		this.config_eavesdrop = config.getBoolean(
				"GriefPrevention.Eavesdrop.Enabled", false);
		outConfig.set("GriefPrevention.Eavesdrop.Enabled",
				this.config_eavesdrop);
		String whisperCommandsToMonitor = config.getString(
				"GriefPrevention.Eavesdrop.WhisperCommands", "/tell;/pm;/r");
		this.config_sign_Eavesdrop = config.getBoolean(
				"GriefPrevention.Eavesdrop.Signs", true);
		outConfig.set("GriefPrevention.Eavesdrop.Signs",
				this.config_sign_Eavesdrop);
		...
		this.config_eavesdrop_bookdrop = config.getBoolean(
				"GriefPrevention.Eavesdrop.BookDrop", false);
		outConfig.set("GriefPrevention.Eavesdrop.BookDrop", false);
		outConfig.set("GriefPrevention.Eavesdrop.WhisperCommands",
				whisperCommandsToMonitor);

		this.config_smartBan = config.getBoolean("GriefPrevention.SmartBan",
				true);
		...
		this.config_creaturesTrampleCrops = config.getBoolean(
				"GriefPrevention.CreaturesTrampleCrops", false);
		this.config_mods_ignoreClaimsAccounts = config
				.getStringList("GriefPrevention.Mods.PlayersIgnoringAllClaims");
		...
		this.config_mods_accessTrustIds = new MaterialCollection(
				GriefPrevention.instance.ModdedBlocks.FoundAccess);
		// build on that list with any configured items.
		//
		List<String> accessTrustStrings = config
				.getStringList("GriefPrevention.Mods.BlockIdsRequiringAccessTrust");
		...
		GriefPrevention.instance.parseMaterialListFromConfig(
				accessTrustStrings, this.config_mods_accessTrustIds);

		//

		// trash blocks.
		this.config_trash_blocks = new MaterialCollection();
		List<String> trashblockStrings = config
				.getStringList("GriefPrevention.TrashBlocks");
		if (GriefPrevention.instance.ModdedBlocks != null) {
		...
		GriefPrevention.instance.parseMaterialListFromConfig(trashblockStrings,
				this.config_trash_blocks);

		this.config_mods_containerTrustIds = new MaterialCollection();
		// Get the config setting....
		List<String> containerTrustStrings = config
				.getStringList("GriefPrevention.Mods.BlockIdsRequiringContainerTrust");
		...
		GriefPrevention.instance.parseMaterialListFromConfig(
				containerTrustStrings, this.config_mods_containerTrustIds);
		...
		this.config_claims_investigationTool = Material
				.getMaterial(investigationToolMaterialName);
		if (this.config_claims_investigationTool == null) {
			GriefPrevention
					.AddLogEntry("ERROR: Material "
							+ investigationToolMaterialName
		...
		this.config_claims_modificationTool = Material
				.getMaterial(modificationToolMaterialName);
		if (this.config_claims_modificationTool == null) {
			GriefPrevention.AddLogEntry("ERROR: Material "
					+ modificationToolMaterialName
					+ " not found.  Defaulting to the golden shovel. ");
			this.config_claims_modificationTool = Material.GOLD_SPADE;
		}
		config_administration_tool = Material.CHAINMAIL_HELMET;
		String admintoolName = config.getString(
				"GriefPrevention.AdministrationTool",
				config_administration_tool.name());
		config_administration_tool = Material.getMaterial(admintoolName);
		if (config_administration_tool == null) {
			GriefPrevention.AddLogEntry("ERROR: Material "
					+ modificationToolMaterialName
					+ " not found.  Defaulting to the chainmail helm.");
		...
		this.config_siege_enabled = config.getBoolean(
				"GriefPrevention.Siege.Enabled", isPvP);
		...
		for (int i = 0; i < this.config_siege_blocks.size(); i++) {
			defaultBreakableBlocksList.add(this.config_siege_blocks.get(i)
					.name());
		}

		// try to load the list from the config file
		List<String> breakableBlocksList = config
				.getStringList("GriefPrevention.Siege.BreakableBlocks");
		...
		for (int i = 0; i < breakableBlocksList.size(); i++) {
			...
			if (material == null) {
				GriefPrevention
						.AddLogEntry("Siege Configuration: Material not found: "
								+ blockName + ".");
			} else {
		...
		outConfig.set("GriefPrevention.Claims.CreationRequiresPermission",
				this.config_claims_creationRequiresPermission);
		outConfig.set("GriefPrevention.Claims.MinimumSize",
				this.config_claims_minSize);
		outConfig.set("GriefPrevention.Claims.MinimumBlocks",
				this.config_claims_minBlocks);
		outConfig.set("GriefPrevention.Claims.MaximumDepth",
				this.config_claims_maxDepth);
		outConfig.set("GriefPrevention.Claims.TrappedCommandCooldownMinutes",
				this.config_claims_trappedCooldownMinutes);
		outConfig.set("GriefPrevention.Claims.InvestigationTool",
				this.config_claims_investigationTool.name());
		outConfig.set("GriefPrevention.Claims.ModificationTool",
				this.config_claims_modificationTool.name());
		outConfig.set("GriefPrevention.Claims.NoSurvivalBuildingOutsideClaims",
				this.config_claims_ApplyTrashBlockRules);
		outConfig.set("GriefPrevention.Claims.WarnWhenBuildingOutsideClaims",
				this.config_claims_warnOnBuildOutside);
		outConfig.set("GriefPrevention.Claims.AllowUnclaimingLand",
				this.config_claims_allowUnclaim);
		outConfig.set("GriefPrevention.Claims.AutoRestoreUnclaimedLand",
				this.config_claims_autoRestoreUnclaimed);

		// outConfig.set("GriefPrevention.Claims.TrashBlocks",trashblocks);
		outConfig.set("GriefPrevention.Claims.WildernessWarningBlockCount",
				this.config_claims_wildernessBlocksDelay);

		// outConfig.set("GriefPrevention.PvP.Worlds", pvpEnabledWorldNames);
		outConfig.set("GriefPrevention.PvP.ProtectFreshSpawns",
				this.config_pvp_protectFreshSpawns);
		outConfig.set("GriefPrevention.PvP.PunishLogout",
				this.config_pvp_punishLogout);
		outConfig.set("GriefPrevention.PvP.CombatTimeoutSeconds",
				this.config_pvp_combatTimeoutSeconds);
		outConfig.set("GriefPrevention.PvP.AllowCombatItemDrop",
				this.config_pvp_allowCombatItemDrop);
		outConfig.set("GriefPrevention.PvP.BlockedSlashCommands",
				bannedPvPCommandsList);
		outConfig
				.set("GriefPrevention.PvP.ProtectPlayersInLandClaims.PlayerOwnedClaims",
						this.config_pvp_noCombatInPlayerLandClaims);
		outConfig
				.set("GriefPrevention.PvP.ProtectPlayersInLandClaims.AdministrativeClaims",
						this.config_pvp_noCombatInAdminLandClaims);

		outConfig.set("GriefPrevention.Trees.RemoveFloatingTreetops",
				this.config_trees_removeFloatingTreetops);
		outConfig.set("GriefPrevention.Trees.RegrowGriefedTrees",
				this.config_trees_regrowGriefedTrees);
		...
		outConfig.set("GriefPrevention.LimitSkyTrees",
				this.config_blockSkyTrees);
		...
		outConfig.set("GriefPrevention.AddItemsToClaimedChests",
		...
		outConfig.set("GriefPrevention.Siege.BreakableBlocks",
				breakableBlocksList);

		outConfig.set("GriefPrevention.CreaturesTrampleCrops",
				this.config_creaturesTrampleCrops);

		outConfig.set("GriefPrevention.Mods.BlockIdsRequiringAccessTrust",
				this.config_mods_accessTrustIds);
		outConfig.set("GriefPrevention.Mods.BlockIdsRequiringContainerTrust",
				this.config_mods_containerTrustIds);
		outConfig.set("GriefPrevention.Mods.BlockIdsExplodable",
				this.config_mods_explodableIds);
		outConfig.set("GriefPrevention.Mods.PlayersIgnoringAllClaims",
				this.config_mods_ignoreClaimsAccounts);
		outConfig.set("GriefPrevention.Mods.BlockIdsRequiringAccessTrust",
				accessTrustStrings);
		outConfig.set("GriefPrevention.Mods.BlockIdsRequiringContainerTrust",
				containerTrustStrings);

		this.config_BlockPlacementRules = BlockPlacementRules.ParseRules(
				config, outConfig, "GriefPrevention.BlockPlacementRules");
		this.config_BlockBreakRules = BlockPlacementRules.ParseRules(config,
				outConfig, "GriefPrevention.BlockBreakRules");
		// outConfig.set("GriefPrevention.Mods.BlockIdsExplodable",
		// explodableStrings);
		...
		if (config_claims_blocksAccruedPerHour > 0
				&& GriefPrevention.instance.ClaimTask == null) {

			GriefPrevention.instance.ClaimTask = new DeliverClaimBlocksTask();
			GriefPrevention.instance
					.getServer()
					.getScheduler()
					.scheduleSyncRepeatingTask(GriefPrevention.instance,
							GriefPrevention.instance.ClaimTask, 60L * 20 * 2,
							60L * 20 * 5);
		}
	...
	public WorldConfig(String worldname) {
		this(worldname, new YamlConfiguration(), ConfigData
				.createTargetConfiguration(worldname));
	}
	...
	public static WorldConfig fromFile(String templateFile) {
		...
		if (!grabfile.exists())
			return null;
		YamlConfiguration Source = YamlConfiguration
				.loadConfiguration(new File(templateFile));
		YamlConfiguration Target = new YamlConfiguration();
		WorldConfig created = new WorldConfig(grabfile.getName(), Source,
				Target);
		try {
