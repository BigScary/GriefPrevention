...
public class BlockPlacementRules {
	...
	public static List<BlockPlacementRules> ParseRules(
			FileConfiguration Source, FileConfiguration Target, String NodePath) {

		ArrayList<BlockPlacementRules> Result = new ArrayList<BlockPlacementRules>();
		ConfigurationSection gotsection = Source
		...
		for (String loopname : sectionnames) {
			Result.add(new BlockPlacementRules(loopname, Source, Target,
					NodePath + "." + loopname));
		}
	...
	public ClaimBehaviourData.ClaimAllowanceConstants checkPlacement(
			Location position, Player p) {
		...
		if (this.MinDistance > 0) {
			ClaimDistanceResult cdr = GriefPrevention.instance.dataStore
					.getNearestClaim(position, this.MinDistance);
			if (p != null) {
				GriefPrevention.sendMessage(p, TextMode.Err,
						Messages.BlockPlacementTooClose,
						String.valueOf(this.MinDistance));
	...
	public BlockPlacementRules(String RuleName, FileConfiguration Source,
			FileConfiguration Target, String NodePath) {
		...
		this.ClaimBehaviour = new ClaimBehaviourData(NodePath + ".Rules",
				Source, Target, NodePath + ".Rules",
				ClaimBehaviourData.getAll(NodePath + ".Rules"));
