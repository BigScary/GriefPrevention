 ...
 * 
 * @author BC_Programming Player groups are defined in the configuration file.
 *         This represents the contents of one group, and provides methods for
 *         matching the items within a group with the names.
 */
public class PlayerGroup {

	private List<String> PlayerNames = new ArrayList<String>(); // names of
																// players in
																// this group.
	 ...
	 *            Config to read from.
	 * @param SourceNode
	 *            Source node to read from within the configuration.
	 * @return
	 */
	public static List<PlayerGroup> getGroups(FileConfiguration Source,
			String SourceNode) {
		...
		ConfigurationSection csec = Source.getConfigurationSection(SourceNode
				+ ".Names");
		if (csec == null) {
			// no groups.
	...
	/**
	 * Saves this PlayerGroup to the given configuration file at the specified
	 * node.
	 * 
	 * @param Target
	 *            Configuration file to save to.
	 * @param TargetNode
	 *            Node to save to in the given configuration file.
	 */
	...
	private PlayerGroup(FileConfiguration Source, String SourceNode) {
		 ...
		 */
		// SourceNode will be "GriefPrevention.Groups.Donator" when loading the
		// Donator Group.

		// First: parse out the Group name. This is everything past the last
		// period.

		String getgroupname = SourceNode
				.substring(SourceNode.lastIndexOf('.') + 1);
