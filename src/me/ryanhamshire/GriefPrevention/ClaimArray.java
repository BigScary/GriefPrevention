...
public class ClaimArray implements Iterable<Claim> {
	...
	private void addClaimWorld(Claim c) {
		...
		Debugger.Write(
				"Claim added to world mapping owned by " + c.getOwnerName()
						+ " to world:" + c.getLesserBoundaryCorner().getWorld(),
				DebugLevel.Verbose);
	...
	private void removeClaimWorld(Claim c) {
		...
		if (claimworldmap.containsKey(usekey)) {
			...
			Debugger.Write(
					"Claim removed from world mapping owned by "
							+ c.getOwnerName() + " to world:"
							+ c.getLesserBoundaryCorner().getWorld(),
					DebugLevel.Verbose);
		}
	...
	public void removeID(Long i) {
		...
		for (String chunk : chunks) {
			ArrayList<Claim> aclaims = chunkmap.get(chunk);
			if (aclaims != null) {
				Debugger.Write("Removing Claim ID #" + i
						+ " From Claim List for Chunk:" + chunk,
						DebugLevel.Verbose);
				aclaims.remove(claim);
				if (aclaims.size() == 0) {
					chunkmap.remove(chunk);
					Debugger.Write(
							"Removing empty chunk mapping entry for chunk "
									+ chunk + " As it now contains no claims.",
							DebugLevel.Verbose);
				}
			}
		}
	...
	public List<Claim> getClaimsInChunk(Location ChunkLocation) {
		...
		String chunkstr = GriefPrevention.instance.dataStore
				.getChunk(ChunkLocation);
