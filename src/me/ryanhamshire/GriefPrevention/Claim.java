...
public class Claim {
	...
	public void removeSurfaceFluids(Claim exclusionClaim) {
		WorldConfig wc = GriefPrevention.instance
				.getWorldCfg(getLesserBoundaryCorner().getWorld());
		...
		if (wc.getWaterBucketEmptyBehaviour()
				.Allowed(getLesserBoundaryCorner(), null).Denied())
			return;
		...
		for (int x = lesser.getBlockX(); x <= greater.getBlockX(); x++) {
			for (int z = lesser.getBlockZ(); z <= greater.getBlockZ(); z++) {
				for (int y = seaLevel - 1; y <= lesser.getWorld()
						.getMaxHeight(); y++) {
					// dodge the exclusion claim
					Block block = lesser.getWorld().getBlockAt(x, y, z);
					if (exclusionClaim != null
							&& exclusionClaim.contains(block.getLocation(),
									true, false))
						continue;
	 ...
	 * @param neverdelete
	 */
	public Claim(Location lesserBoundaryCorner, Location greaterBoundaryCorner,
			String ownerName, String[] builderNames, String[] containerNames,
			String[] accessorNames, String[] managerNames, Long id,
			boolean neverdelete) {
		...
		for (int i = 0; i < builderNames.length; i++) {
			...
			if (name != null && !name.isEmpty()) {
				this.playerNameToClaimPermissionMap.put(name,
	 ...
	 */
	public int getArea() {
		int claimWidth = this.greaterBoundaryCorner.getBlockX()
				- this.lesserBoundaryCorner.getBlockX() + 1;
		int claimHeight = this.greaterBoundaryCorner.getBlockZ()
				- this.lesserBoundaryCorner.getBlockZ() + 1;
	...
	public void setLocation(Location FirstBorder, Location SecondBorder) {
		...
		Location FirstPos = new Location(FirstBorder.getWorld(), MinX, MinY,
				MinZ);
		Location SecondPos = new Location(FirstBorder.getWorld(), MaxX, MaxY,
				MaxZ);
	 ...
	 */
	public int getWidth() {
		return this.greaterBoundaryCorner.getBlockX()
				- this.lesserBoundaryCorner.getBlockX() + 1;
	}
	...
	public int getHeight() {
		return this.greaterBoundaryCorner.getBlockZ()
				- this.lesserBoundaryCorner.getBlockZ() + 1;
	}
	...
	public String allowEdit(Player player) {
		...
		if (this.ownerName.equals(player.getName())) {
			if (this.siegeData != null) {
				return GriefPrevention.instance.dataStore
						.getMessage(Messages.NoModifyDuringSiege);
			}
		...
		return GriefPrevention.instance.dataStore.getMessage(
				Messages.OnlyOwnersModifyClaims, this.getOwnerName());
	}
	...
	public String allowBuild(Player player) {
		...
		if (playerData.inPvpCombat()) {
			return GriefPrevention.instance.dataStore
					.getMessage(Messages.NoBuildPvP);
		}

		// owners can make changes, or admins with ignore claims mode enabled
		if (this.ownerName.equals(player.getName())
				|| GriefPrevention.instance.dataStore.getPlayerData(player
						.getName()).ignoreClaims)
			return null;
		...
		// permission
		ClaimPermission permissionLevel = this.playerNameToClaimPermissionMap
				.get("public");
		...
		String reason = GriefPrevention.instance.dataStore.getMessage(
				Messages.NoBuildPermission, this.getOwnerName());
	...
	private boolean isApplicablePlayer(String identifier, String pName) {
		...
		if (identifier.toUpperCase().startsWith("G:")) {
			...
			PlayerGroup FoundGroup = GriefPrevention.instance.config_player_groups
					.getGroupByName(identifier);
			if (FoundGroup == null)
	...
	private boolean hasExplicitPermission(Player player, ClaimPermission level) {
		...
		while (iterator.hasNext()) {
			...
			if (isApplicablePlayer(identifier, playerName)
					&& this.playerNameToClaimPermissionMap.get(identifier) == level) {
				...
				// drop the brackets
				String permissionIdentifier = identifier.substring(1,
						identifier.length() - 1);
	...
	public String allowBreak(Player player, Block BlocktoCheck) {
		...
		if (this.siegeData != null) {
			...
			breakable = breakable
					|| BrokenBlockInfo.canBreak(BlocktoCheck.getLocation());
			...
			if (breakable
				...
				if (wc.getSiegeBlockRevert()) {
					...
					// with air ourselves.
					String usekey = GriefPrevention
							.getfriendlyLocationString(BlocktoCheck
									.getLocation());
					...
					if (siegeData.SiegedBlocks.containsKey(usekey)) {
						...
						// list.
						siegeData.SiegedBlocks.put(usekey, new BrokenBlockInfo(
								BlocktoCheck.getLocation()));
						// replace it manually
			...
			if (!breakable) {
				return GriefPrevention.instance.dataStore
						.getMessage(Messages.NonSiegeMaterial);
			} else if (this.ownerName.equals(player.getName())) {
				return GriefPrevention.instance.dataStore
						.getMessage(Messages.NoOwnerBuildUnderSiege);
			} else {
	...
	public String allowAccess(Player player) {
		...
		ClaimPermission permissionLevel = this.playerNameToClaimPermissionMap
				.get("public");
		if (ClaimPermission.Build == permissionLevel
				|| ClaimPermission.Inventory == permissionLevel
		...
		String reason = GriefPrevention.instance.dataStore.getMessage(
				Messages.NoAccessPermission, this.getOwnerName());
		if (player.hasPermission("griefprevention.ignoreclaims"))
			reason += "  "
					+ GriefPrevention.instance.dataStore
							.getMessage(Messages.IgnoreClaimsAdvertisement);
	...
	public String allowContainers(Player player) {
		...
		if (this.siegeData != null) {
			return GriefPrevention.instance.dataStore.getMessage(
					Messages.NoContainersSiege, siegeData.attacker.getName());
		}
		...
		String reason = GriefPrevention.instance.dataStore.getMessage(
				Messages.NoContainersPermission, this.getOwnerName());
		if (player.hasPermission("griefprevention.ignoreclaims"))
			reason += "  "
					+ GriefPrevention.instance.dataStore
							.getMessage(Messages.IgnoreClaimsAdvertisement);
	...
	public String allowGrantPermission(Player player) {
		...
		String reason = GriefPrevention.instance.dataStore.getMessage(
				Messages.NoPermissionTrust, this.getOwnerName());
		if (player.hasPermission("griefprevention.ignoreclaims"))
			reason += "  "
					+ GriefPrevention.instance.dataStore
							.getMessage(Messages.IgnoreClaimsAdvertisement);
	 ...
	 * @return
	 */
	public boolean setPermission(String playerName,
		...
		if (inDataStore) {
			...
			ClaimModifiedEvent claimevent = new ClaimModifiedEvent(this, null,
					permtype);
		...
		this.playerNameToClaimPermissionMap.put(playerName.toLowerCase(),
				permissionLevel);
		return true;
	}
	...
	public boolean dropPermission(String playerName) {
		// we only want to send events if the claim is in the data store
		if (inDataStore) {
			ClaimPermission perm = this.playerNameToClaimPermissionMap
	...
	public boolean clearPermissions() {
		...
		if (inDataStore) {
			ClaimModifiedEvent claimevent = new ClaimModifiedEvent(this, null,
					ClaimModifiedEvent.Type.PermissionsCleared);
			Bukkit.getServer().getPluginManager().callEvent(claimevent);
			if (claimevent.isCancelled()) {
				return false;
			}
	 ...
	 * @param managers
	 */
	public void getPermissions(ArrayList<String> builders,
			ArrayList<String> containers, ArrayList<String> accessors,
			ArrayList<String> managers) {
		getPermissions((List<String>) builders, (List<String>) containers,
				(List<String>) accessors, (List<String>) managers);
	}
	 ...
	 * @param managers
	 */
	public void getPermissions(List<String> builders, List<String> containers,
			List<String> accessors, List<String> managers) {
		// loop through all the entries in the hash map
		// if we have a parent, add the parent permissions first, then overwrite
		// them.
		Iterator<Map.Entry<String, ClaimPermission>> mappingsIterator = this.playerNameToClaimPermissionMap
				.entrySet().iterator();
	...
	public String getOwnerName() {
		...
		if (this.ownerName.length() == 0)
			return GriefPrevention.instance.dataStore
					.getMessage(Messages.OwnerNameForAdminClaims);

		return this.ownerName;
	}

	public boolean contains(Claim otherclaim, boolean ignoreHeight) {
		return (contains(otherclaim.lesserBoundaryCorner, ignoreHeight, false) && contains(
				otherclaim.greaterBoundaryCorner, ignoreHeight, false));
	}

	public static boolean Contains(Location pA, Location pB, Location Target,
	...
	public boolean contains(Location location, boolean ignoreHeight,
		...
		// main check
		boolean inClaim = (ignoreHeight || y >= this.lesserBoundaryCorner
				.getBlockY())
	...
	boolean overlaps(Claim otherClaim) {
		...
		if (!this.lesserBoundaryCorner.getWorld().equals(
				otherClaim.getLesserBoundaryCorner().getWorld()))
			return false;
		...
		if (otherClaim.contains(
				new Location(this.lesserBoundaryCorner.getWorld(),
						this.lesserBoundaryCorner.getBlockX(), 0,
						this.greaterBoundaryCorner.getBlockZ()), true, false))
			return true;
		if (otherClaim.contains(
				new Location(this.lesserBoundaryCorner.getWorld(),
						this.greaterBoundaryCorner.getBlockX(), 0,
						this.lesserBoundaryCorner.getBlockZ()), true, false))
			return true;
		...
		if (this.getLesserBoundaryCorner().getBlockZ() <= otherClaim
				.getGreaterBoundaryCorner().getBlockZ()
				&& this.getLesserBoundaryCorner().getBlockZ() >= otherClaim
						.getLesserBoundaryCorner().getBlockZ()
				&& this.getLesserBoundaryCorner().getBlockX() < otherClaim
						.getLesserBoundaryCorner().getBlockX()
				&& this.getGreaterBoundaryCorner().getBlockX() > otherClaim
						.getGreaterBoundaryCorner().getBlockX())
			return true;

		if (this.getGreaterBoundaryCorner().getBlockZ() <= otherClaim
				.getGreaterBoundaryCorner().getBlockZ()
				&& this.getGreaterBoundaryCorner().getBlockZ() >= otherClaim
						.getLesserBoundaryCorner().getBlockZ()
				&& this.getLesserBoundaryCorner().getBlockX() < otherClaim
						.getLesserBoundaryCorner().getBlockX()
				&& this.getGreaterBoundaryCorner().getBlockX() > otherClaim
		...
		if (this.getLesserBoundaryCorner().getBlockX() <= otherClaim
				.getGreaterBoundaryCorner().getBlockX()
				&& this.getLesserBoundaryCorner().getBlockX() >= otherClaim
						.getLesserBoundaryCorner().getBlockX()
				&& this.getLesserBoundaryCorner().getBlockZ() < otherClaim
		...
		if (this.getGreaterBoundaryCorner().getBlockX() <= otherClaim
				.getGreaterBoundaryCorner().getBlockX()
				&& this.getGreaterBoundaryCorner().getBlockX() >= otherClaim
						.getLesserBoundaryCorner().getBlockX()
				&& this.getLesserBoundaryCorner().getBlockZ() < otherClaim
	...
	public String allowMoreEntities() {
		...
		if (!GriefPrevention.instance.creativeRulesApply(this
		...
		if (maxEntities == 0)
			return GriefPrevention.instance.dataStore
					.getMessage(Messages.ClaimTooSmallForEntities);
			...
			for (int z = lesserChunk.getZ(); z <= greaterChunk.getZ(); z++) {
				...
				for (int i = 0; i < entities.length; i++) {
					...
					if (!(entity instanceof Player)
							&& this.contains(entity.getLocation(), false, false)) {
						totalEntities++;
		...
		if (totalEntities > maxEntities)
			return GriefPrevention.instance.dataStore
					.getMessage(Messages.TooManyEntitiesInClaim);
	...
	boolean greaterThan(Claim otherClaim) {
		...
		return thisCorner.getWorld().getName()
				.compareTo(otherCorner.getWorld().getName()) < 0;
	}
	...
	public long getPlayerInvestmentScore() {
		...
		ArrayList<Integer> playerBlocks = RestoreNatureProcessingTask
				.getPlayerBlocks(lesserBoundaryCorner.getWorld()
						.getEnvironment(), lesserBoundaryCorner.getBlock()
						.getBiome());

		// scan the claim for player placed blocks
		double score = 0;

		boolean creativeMode = GriefPrevention.instance
				.creativeRulesApply(lesserBoundaryCorner);

		for (int x = this.lesserBoundaryCorner.getBlockX(); x <= this.greaterBoundaryCorner
				.getBlockX(); x++) {
			for (int z = this.lesserBoundaryCorner.getBlockZ(); z <= this.greaterBoundaryCorner
					.getBlockZ(); z++) {
				int y = this.lesserBoundaryCorner.getBlockY();
				for (; y < GriefPrevention.instance
						.getSeaLevel(this.lesserBoundaryCorner.getWorld()) - 5; y++) {
					Block block = this.lesserBoundaryCorner.getWorld()
							.getBlockAt(x, y, z);
					...
					if (playerBlocks.contains(block.getTypeId())) {
						if (block.getType() == Material.CHEST && !creativeMode) {
							score += 10;
								...
								&& (block.getType() == Material.LAVA || block
	...
	public boolean addManager(String player) {
		// we only want to send events if the claim is in the data store
		if (inDataStore) {
			ClaimModifiedEvent claimevent = new ClaimModifiedEvent(this,
					player, ClaimModifiedEvent.Type.AddedManager);
			Bukkit.getServer().getPluginManager().callEvent(claimevent);
			if (claimevent.isCancelled()) {
				return false;
			}
		}
		System.out.println("Adding " + player + " as manager to claim ID #"
				+ this.getID());
		managers.add(player);
		if (inDataStore)
	...
	public boolean removeManager(String player) {
		...
		if (inDataStore) {
			ClaimModifiedEvent claimevent = new ClaimModifiedEvent(this,
					player, ClaimModifiedEvent.Type.RemovedManager);
			Bukkit.getServer().getPluginManager().callEvent(claimevent);
			if (claimevent.isCancelled()) {
				return false;
			}
	...
	public FileConfiguration getPluginMetadata(String PluginKey) {
		return GriefPrevention.instance.getMetaHandler().getClaimMeta(
				PluginKey, this);
	}
