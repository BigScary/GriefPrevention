...
class EntityEventHandler implements Listener {
	...
	public void onEntityChangeBlock(EntityChangeBlockEvent event) {
		...
		if (event.getEntityType() == EntityType.ENDERMAN) {
			...
			if (theenderman.getCarriedMaterial() == null
					|| theenderman.getCarriedMaterial().getItemType() == Material.AIR) {
				// carrying nothing, so picking up. This could be reversed
				// depending on when this event is
				// actually called...
				ClaimAllowanceConstants cac = wc.getEndermanPickupRules()
						.Allowed(event.getBlock().getLocation(), null);
				if (cac.Allowed())
					return;
				...
				ClaimAllowanceConstants cac = wc.getEndermanPlacementRules()
						.Allowed(event.getBlock().getLocation(), null);
				if (cac.Allowed())
					return;
				event.setCancelled(true);
			...
			if (wc.getSilverfishBreakRules()
					.Allowed(
							event.getEntity().getLocation().getBlock()
									.getLocation(), null).Denied())
				event.setCancelled(true);
			return;
		}
		...
		// determined, too expensive to constantly check for claimed blocks
			...
			event.setCancelled(wc.getWitherEatBehaviour()
					.Allowed(event.getEntity().getLocation(), null).Denied());
			return;
		}
	}
	...
	public void onZombieBreakDoor(EntityBreakDoorEvent event) {
		...
		if (!wc.getClaimsEnabled())
			return;
		if (!wc.getZombieDoorBreaking()
				.Allowed(event.getEntity().getLocation(), null).Allowed())
			event.setCancelled(true);
	}
	...
	public void onEntityInteract(EntityInteractEvent event) {
		...
		if (event.getEntity() instanceof Arrow) {
			...
			if (event.getBlock().getType() == Material.WOOD_BUTTON) {
				if (wc.getArrowWoodenButtonRules()
						.Allowed(event.getBlock().getLocation(), source)
						.Denied()) {
					event.setCancelled(true);
				...
				if (wc.getArrowWoodenTouchPlateRules()
						.Allowed(event.getBlock().getLocation(), source)
						.Denied()) {
					event.setCancelled(true);
					proj.remove();
			...
			grabplayer = event.getEntity() instanceof Player ? (Player) event
					.getEntity() : null;
			if (grabplayer != null) {
		...
		if (grabplayer != null)
			pdata = GriefPrevention.instance.dataStore.getPlayerData(grabplayer
					.getName());
		if (event.getBlock().getType() == Material.STONE_PLATE) {
			if (grabplayer != null) {
				pdata.PlateData.put(event.getBlock().getLocation().toString(),
						pdata.new PressurePlateData(event.getBlock()
								.getLocation()));
			}
			if (wc.getStonePressurePlates()
					.Allowed(event.getBlock().getLocation(), grabplayer)
					.Denied()) {
				event.setCancelled(true);
	...
	public void onEntityExplode(EntityExplodeEvent explodeEvent) {
		...
		WorldConfig wc = GriefPrevention.instance.getWorldCfg(location
				.getWorld());
		if (!wc.getClaimsEnabled())
			return;
		Claim claimatEntity = GriefPrevention.instance.dataStore.getClaimAt(
				location, true);
		...
		boolean isCreeper = explodingEntity != null
				&& explodingEntity instanceof Creeper;
	...
	public void onItemSpawn(ItemSpawnEvent event) {
		...
		WorldConfig wc = GriefPrevention.instance.getWorldCfg(event
		...
		if (GriefPrevention.instance.creativeRulesApply(event.getLocation())) {
			...
			if (event.getEntity().getItemStack().getType() == Material.WRITTEN_BOOK) {
				...
				if (wc.getEavesDropBookDrop()) {
					...
					Player DroppingPlayer = GriefPrevention.instance
							.getNearestPlayer(event.getEntity().getLocation());
					StringBuffer buildMessage = new StringBuffer();

					BookMeta bm = (BookMeta) event.getEntity().getItemStack()
							.getItemMeta();

					// bm.getTitle()
					buildMessage.append("Book Dropped by "
							+ DroppingPlayer.getName() + " Titled:"
							+ bm.getTitle());

					GriefPrevention.sendEavesDropMessage(DroppingPlayer,
	...
	public void onExpBottle(ExpBottleEvent event) {
		...
		// ground)
		if (GriefPrevention.instance.creativeRulesApply(event.getEntity()
				.getLocation())) {
	...
	public void onEntitySpawn(CreatureSpawnEvent event) {
		...
		if (reason == SpawnReason.BUILD_WITHER) {
			...
			if (wc.getWitherSpawnBehaviour()
					.Allowed(entity.getLocation(), null).Denied()) {
				event.setCancelled(true);
			...
			if (wc.getSnowGolemSpawnBehaviour()
					.Allowed(entity.getLocation(), null).Denied()) {
				event.setCancelled(true);
			...
			if (wc.getIronGolemSpawnBehaviour()
					.Allowed(entity.getLocation(), null).Denied()) {
				event.setCancelled(true);
	...
	public void onEntityDeath(EntityDeathEvent event) {
		...
		if (entity instanceof Player) {
			...
			if (playerData.siegeData != null) {
				...
				this.dataStore.endSiege(playerData.siegeData, null,
						player.getName(), true /* ended due to death */);
				return;
			}
		}
		...
		// inform the owner of the casualty on the line of battle.
			...
			if (entity instanceof Tameable) {
				...
				if (tamed.isTamed()) {
					...
					PlayerData ownerdata = GriefPrevention.instance.dataStore
							.getPlayerData(ownername);
					if (ownerdata != null) {
						if (ownerdata.siegeData != null) {
							// if the owner is the Defender...
							if (ownerdata.siegeData.defender == tamed
									.getOwner()) {
								...
								if (tamed.getOwner() instanceof Player) {
									...
									Player theplayer = (Player) tamed
											.getOwner();
									GriefPrevention.sendMessage(theplayer,
											TextMode.Info,
											Messages.TamedDeathDefend,
											tamedname);
	...
	public void onEntityPickup(EntityChangeBlockEvent event) {
		...
		if (event.getEntity() instanceof Enderman) {
			if (wc.getEndermanPickupRules()
					.Allowed(event.getBlock().getLocation(), null).Denied()) {
				event.setCancelled(true);
			}
	...
	public void onHangingBreak(HangingBreakEvent event) {
		...
		String noBuildReason = GriefPrevention.instance.allowBuild(
				playerRemover, event.getEntity().getLocation());
		if (noBuildReason != null) {
			event.setCancelled(true);
			GriefPrevention.sendMessage(playerRemover, TextMode.Err,
					noBuildReason);
		}
	}
	...
	public void onPaintingPlace(HangingPlaceEvent event) {
		...
		String noBuildReason = GriefPrevention.instance.allowBuild(
				event.getPlayer(), event.getEntity().getLocation());
		if (noBuildReason != null) {
			event.setCancelled(true);
		...
		else if (GriefPrevention.instance.creativeRulesApply(event.getEntity()
				.getLocation())) {
			PlayerData playerData = this.dataStore.getPlayerData(event
					.getPlayer().getName());
			Claim claim = this.dataStore.getClaimAt(event.getBlock()
					.getLocation(), false);
			if (claim == null)
				return;
	...
	public void onEntityDamage(EntityDamageEvent event) {
		...
		if (event.getEntity() instanceof Hanging) { // hanging objects are not
													...
													// inside claims.
			Claim claimatpos = dataStore.getClaimAt(event.getEntity()
					.getLocation(), false);
			if (claimatpos != null) {
		...
		if (attacker != null && event.getEntity() instanceof Player
				&& event.getEntity().getWorld().getPVP()) {
			...
			PlayerData defenderData = this.dataStore
					.getPlayerData(((Player) event.getEntity()).getName());
			...
			if (wc.getPvPNoCombatinPlayerClaims()
					|| wc.getNoPvPCombatinAdminClaims()) {
				Claim attackerClaim = this.dataStore.getClaimAt(
						attacker.getLocation(), false);
				if (attackerClaim != null
						&& (attackerClaim.isAdminClaim()
								&& wc.getNoPvPCombatinAdminClaims() || !attackerClaim
								.isAdminClaim()
								&& wc.getPvPNoCombatinPlayerClaims())) {
				...
				Claim defenderClaim = this.dataStore.getClaimAt(
						defender.getLocation(), false);
				if (defenderClaim != null
						&& (defenderClaim.isAdminClaim()
								&& wc.getNoPvPCombatinAdminClaims() || !defenderClaim
								.isAdminClaim()
								&& wc.getPvPNoCombatinPlayerClaims())) {
		...
		if (event instanceof EntityDamageByEntityEvent) {
			...
			if (subEvent.getEntity() instanceof Creature) {
				...
				Claim claim = this.dataStore.getClaimAt(event.getEntity()
						.getLocation(), false);
				...
				if (claim != null) {
					...
					if (attacker == null) {
						...
						if (event.getEntity() instanceof Villager
								&& damageSource instanceof Monster
								&& claim.isAdminClaim()) {
							return;
					...
					// sieges.
						...
						if (event.getEntityType() == EntityType.WOLF
							...
							Claim mobclaim = GriefPrevention.instance.dataStore
									.getClaimAt(
							...
							if (mobclaim != null && mobclaim.siegeData != null) {
								...
								if (attacker.getLocation().distance(
										defender.getLocation()) < wc
										.getSiegeTamedAnimalDistance()) {
	...
	public void onVehicleDamage(VehicleDamageEvent event) {
		...
		// don't cancel the event.
				...
				&& wc.getEnvironmentalVehicleDamage()
						.Allowed(event.getVehicle().getLocation(), attacker,
								false).Allowed()) {
			return;
		}
		...
		Claim claim = this.dataStore.getClaimAt(event.getVehicle()
				.getLocation(), false);
