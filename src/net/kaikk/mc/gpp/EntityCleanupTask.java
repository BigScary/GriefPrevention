...
class EntityCleanupTask implements Runnable 
{
	...
	public void run() 
	{
		...
		for(World world : GriefPreventionPlus.instance.getServer().getWorlds())
	    {
		...
		for(int i = 0; i < worlds.size(); i++)
		{
			...
			for(; j < entities.size() && j < k; j++)
			{
				...
				{
					Claim claim = GriefPreventionPlus.instance.dataStore.getClaimAt(entity.getLocation(), false, cachedClaim);
					if(claim != null)
					{
						cachedClaim = claim;
					}
		...
		GriefPreventionPlus.instance.getServer().getScheduler().scheduleSyncDelayedTask(GriefPreventionPlus.instance, task, 20L * 60 * 1);
	}
}
