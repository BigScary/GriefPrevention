...
class UUIDFetcher {
    ...
    public void call() throws Exception
    {
        ...
        GriefPreventionPlus.AddLogEntry("UUID conversion process started.  Please be patient - this may take a while.");
        
        GriefPreventionPlus.AddLogEntry("Mining your local world data to save calls to Mojang...");
        OfflinePlayer [] players = GriefPreventionPlus.instance.getServer().getOfflinePlayers();
        ...
        GriefPreventionPlus.AddLogEntry("Checking local server data to get correct casing for player names...");
        for(int i = 0; i < names.size(); i++)
        {
            ...
            {
                GriefPreventionPlus.AddLogEntry(name + " --> " + correctCasingName);
                names.set(i, correctCasingName); 
            }
        }
        
        //look for local uuid's first
        GriefPreventionPlus.AddLogEntry("Checking local server data for UUIDs already seen...");
        for(int i = 0; i < names.size(); i++)
        {
            String name = names.get(i);
            ...
            if(uuid != null)
            {
                GriefPreventionPlus.AddLogEntry(name + " --> " + uuid.toString());
                names.remove(i--);
            }
        }
        
        //for online mode, call Mojang to resolve the rest
        if(GriefPreventionPlus.instance.getServer().getOnlineMode())
        {
            GriefPreventionPlus.AddLogEntry("Calling Mojang to get UUIDs for remaining unresolved players (this is the slowest step)...");
                ...
                do
                {
                    ...
                    catch(Exception e)
                    {
                        ...
                        if(e.getMessage().contains("429"))
                        {
                            ...
                            {
                                GriefPreventionPlus.AddLogEntry("Batch size " + PROFILES_PER_REQUEST + " seems too large.  Looking for a workable batch size...");
                            ...
                            {
                                GriefPreventionPlus.AddLogEntry("Mojang says we're sending requests too fast.  Will retry every 30 seconds until we succeed...");
                                Thread.sleep(30000);
                            }
                        }
                        else
        ...
        {
            GriefPreventionPlus.AddLogEntry("Generating offline mode UUIDs for remaining unresolved players...");
